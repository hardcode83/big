...
- [x] whatsapp-dev-credentials-render — [INFRA] **que el `.env` que el CD renderiza en la VM lleve las cinco `WHATSAPP_*`** … → changes/archive/2026-09-12-whatsapp-dev-credentials-render/
...
- [x] staff-messaging-web — [FE] **el hilo del personal donde ya se trabaja**: `/cleaner/tasks/[id]` y `/tech/incidents/[id]`. Va detrás de `cleaner-app` y `tech-app`, que son las que estrenan esas páginas — hoy son `RoutePlaceholder` (no está en el plan original, añadida el 2026-08-28) … **la vista del manager sobre el hilo quedó fuera de esta entrega** (proposal.md §Out of scope, para no dar cobertura asimétrica mientras el manager no tenga `/cleaning/[id]`) — ver `staff-messaging-manager-view` → changes/archive/2026-09-17-staff-messaging-web/
  needs: staff-messaging, cleaner-app, tech-app · size: M · kind: feature
- [x] cleaning-manager-task-detail — [FE] **el manager no tiene página de detalle de una tarea de limpieza** (`/cleaning/[id]`): `cleaning-manager-view` solo entregó el listado (`/cleaning`) y dejó su propia página de detalle para «una entrada propia del roadmap cuando exista la superficie de detalle». Es el prerrequisito compartido de dos huecos encontrados el mismo día auditando `staff-messaging-web`: las fotos de limpieza del manager (`photo-storage-manager-view`) y el hilo de mensajería del manager sobre una limpieza (`staff-messaging-manager-view`) — ninguno de los dos tiene dónde montarse sin esta página (no está en el plan original, añadida el 2026-09-17 al archivar `staff-messaging-web`) → changes/archive/2026-09-18-cleaning-manager-task-detail/
  needs: cleaning-manager-view · size: S · kind: feature
- [ ] staff-messaging-manager-view — [FE] **la vista del manager sobre el hilo de mensajería**, que `staff-messaging-web` recortó explícitamente: el backend ya autoriza a `PROPERTY_MANAGER` a leer y escribir ambos hilos (D3 de `staff-messaging`), y `/incidents/[id]` ya existe como página real, pero construirlo solo para incidencias dejaría cobertura asimétrica frente a `cleaning` mientras no exista `/cleaning/[id]` (no está en el plan original, añadida el 2026-09-17 al archivar `staff-messaging-web`)
  needs: staff-messaging-web, incident-triage-web, cleaning-manager-task-detail · size: S · kind: feature
- [x] photo-storage-manager-view — [FE] **las fotos de incidencia/limpieza sin destino en el manager, y su almacenamiento sin garantía fuera de `LOCAL`**: el backend ya expone `GET /incidents/{id}/photos` y `GET /cleaning-tasks/{task_id}/photos` para `PROPERTY_MANAGER` (mismos permisos `READ_INCIDENTS`/`READ_CLEANING_TASKS`, sin permiso nuevo) y el frontend hasta tiene `useIncidentPhotos()` sin ningún consumidor, pero ninguna pantalla del manager las pinta — excluido explícitamente tres veces seguidas (`cleaning-manager-view`, `incident-triage-web`, `staff-messaging-web`) sin que nadie abriera la entrada prometida. Además, `bootstrap_storage_type` (default `LOCAL`) solo se fuerza a `S3` pasándolo en línea a mano (`RUNBOOK.md`, `-e BOOTSTRAP_STORAGE_TYPE=S3`); ningún gate impide que un tenant nuevo nazca en `LOCAL` en `dev`/`staging`/`production` si alguien olvida el flag, pese a que `settings.environment` ya distingue `local` del resto (`config.py`). El aislamiento por tenant en la clave del objeto (`tenants/{tenant_id}/...`, `specs/file-storage.md`) ya existe — por verificar, no por construir (no está en el plan original, añadida el 2026-09-17 al auditar `staff-messaging-web`) → changes/archive/2026-09-18-photo-storage-manager-view/
  needs: incident-photos, cleaning-photos-storage, object-storage-provisioning, cleaning-manager-view, incident-triage-web, cleaning-manager-task-detail · size: M · kind: feature
...
- [ ] audit-changes-repository-guard — [BE] **cerrar `audit_logs.changes` en el repositorio, no solo en `ChangeSet`** …
  size: S · kind: tech
- [x] validation-error-loc-redaction — [BE] **acotar el `loc` que el 422 de validación devuelve al llamante** … → changes/archive/2026-09-15-validation-error-loc-redaction/
  size: S · kind: tech
...
- [x] cleaner-list-property-projection — [BE] **que el listado de tareas de limpieza traiga su vivienda, en vez de que cada fila la vaya a buscar** … → changes/archive/2026-09-18-cleaner-list-property-projection/
...
- [x] reservation-confirm-web — [FE] **confirmar o rechazar una reserva `PENDING`, y registrar su señal de pago, desde `/reservations/[id]`** — lo único que `reservation-create-web` dejó sin mover: el backend admite `PATCH {"status": "CONFIRMED"}` sin restricción de transición (`schemas.py:150-176`), pero `EDITABLE_FIELDS` del formulario de edición excluye `status` y `payment_status` a propósito (`edit-reservation-form.tsx:17-20`, fijado por su propio test) y ningún control las dispara. Sin ella, ninguna reserva directa alcanza el reloj de estados (`sim-advance`/`beat` sólo mueven `CONFIRMED`) sin un `PATCH` manual por API — el mismo patrón que usa `seed-data-demo-extension` para sus estancias sembradas. Investigado el 2026-09-12 contra la práctica real del sector (ventana de 24h del host en Airbnb/Booking "Request to Book", no ligada a la fecha de check-in; arras/señal 20-30% y aprobación manual en gestión directa, patrón Guesty/OwnerRez) antes de decidir el mecanismo: confirmación **manual y desacoplada del pago** — un botón que el manager acciona con su propio criterio — con `payment_status` expuesto en el mismo formulario como registro, no como gate; rechazar reutiliza el `DELETE`/cancelar ya existente (no está en el plan original, añadida el 2026-09-12 al auditar el ciclo completo tras cerrar `reservation-create-web`; hito «MVP operable» 1) → changes/archive/2026-09-13-reservation-confirm-web/
  needs: reservation-create-web · size: S · kind: feature
