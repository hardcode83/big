...
- [x] ci-runner-pool-oci — [INFRA] escalar el runner único de `ci-runner-oci` a un pool de N agentes en la misma VM (`runner_count`, default 4), para que dos jobs `pull_request`-triggered coincidentes se repartan entre agentes en vez de … → changes/archive/2026-09-04-ci-runner-pool-oci/
  size: S · kind: infra
- [ ] ci-runner-workspace-pollution — [INFRA] **los contenedores del stack local dejan ficheros de root en el árbol del repo, y en la VM de CI ese árbol es el `_work/` persistente del agente**: `docker-compose.yml` monta `./backend:/app` en `migrate`, `backend`, `worker` y `beat`, ninguno declara `user:`, así que al importar código Python escriben `__pycache__/*.pyc` propiedad de root sobre el host. `actions/checkout` (`clean: true`) no puede borrarlos y falla con `EACCES` **en el paso 0**, matando el job antes de que ejecute nada suyo — por eso ningún paso del workflow puede mitigarlo, a diferencia del EACCES gemelo de `frontend/node_modules` que `hardening-release` resolvió moviendo un paso dentro del job. El 2026-09-14 tumbó `deploy-dev` de los PR #197 y #196 (dev se quedó congelado en el deploy del PR #194, del 2026-09-13), `api-contract`/`e2e-tests`/`backend-tests-detect` del PR #196, y el `demo-reset` programado; tres de los cuatro agentes estaban contaminados (`actions-runner-2` con 25 directorios, `-3` con 2, `-1` con el que disparó el diagnóstico) y sólo `-4` quedaba limpio. Dos capas: que el stack deje de generar esos ficheros, y un `ACTIONS_RUNNER_HOOK_JOB_STARTED` versionado en `runner-bootstrap.sh` que limpie restos de root antes de cada job — la única defensa posible contra un fichero de root futuro de otra fuente, y además lo que exige la norma IaC-first frente a la limpieza manual por SSH con que se desatascó (no está en el plan original, añadida el 2026-09-14 tras el incidente que siguió al merge de `hardening-release`)
  needs: ci-runner-pool-oci · size: S · kind: fix
