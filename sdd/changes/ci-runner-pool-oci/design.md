...
### D7 — Verificación runner-side, aceptada como gate post-merge

**Chosen:** la sección Verification de `tasks.md` exige (a) `gh api ... /repos/{owner}/{repo}/actions/runners` listando exactamente N entradas con label `dev`, nombre `autohostai-dev-vm-<i>` para `i ∈ [1..N]`, y status `online`, **sin** la entrada legada `autohostai-dev-vm`; y (b) N jobs de `api-contract.yml` lanzados por `workflow_dispatch`, **no simultáneos**, espaciados 60s entre cada uno (la API de GitHub no permite seleccionar un runner por nombre desde `runs-on:`; solo por label). Cada job se rastrea por su `actions_run_id`; para cada uno, `gh api ... /repos/{owner}/{repo}/actions/runs/<id>` devuelve el campo `runner.name`, y la verificación acepta solo si los N jobs acabaron en `success` **y** cada uno corrió en un `runner.name` distinto (un solo job no puede agotar el pool: si todos van al agente 1, GitHub no rotará por sí mismo si los demás están `online`/`idle`, así que se inserta el `sleep 60` para que el primero termine antes de lanzar el segundo). El resultado es una matriz (job → runner.name) que prueba que GitHub repartió entre los N agentes.

**Ambas partes de esta verificación son estructuralmente post-merge, no un gate previo al PR (enmienda 2026-09-04)**: (a) requiere que los N agentes existan de verdad en la VM, y el único canal para llevarlos ahí es el manual de `RUNBOOK.md §6.2` (SSH + `runner-bootstrap.sh` actualizado) — **no** `terraform apply`, porque la instancia declara `ignore_changes = [metadata, ...]` (`main.tf:192`), así que un apply nunca reescribe el `user_data`/cloud-init de una VM ya viva; ejecutar ese paso manual con el código de una rama sin mergear desplegaría un bootstrap no revisado en la infra compartida de CI, exactamente el riesgo que la revisión del PR existe para evitar. (b) depende de que (a) ya haya corrido. Mismo patrón que `ci-runner-oci` aceptó para sus 6 jobs gateados a `main` (`deploy-dev`: build/deploy; `infra-dev`: plan/apply/check): la primera evidencia real es post-merge, tratada como **gate vigilado explícito con rollback inmediato si falla por algo que la revisión estática no habría revelado** — no un requisito que bloquee la apertura del PR.
...
Rejected: intentar `terraform apply` desde la rama del PR para producir esta evidencia antes de mergear — `infra-dev.yml`'s job `apply` está gateado a `github.ref == 'refs/heads/main'` (`steering/infra.md`), así que GitHub no lo dispararía en absoluto sobre este PR, y aunque se forzara, `ignore_changes` en la instancia lo dejaría sin efecto sobre la VM viva.
