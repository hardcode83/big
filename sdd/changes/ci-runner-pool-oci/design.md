...
### D2 — `runner_count` como variable de Terraform, default 2

**Chosen:** nueva `variable "runner_count"` (`type=number, default=2, validation: 1 ≤ runner_count ≤ 4`) en `infra/environments/dev/variables.tf`. El valor se inyecta al `cloud-init.yaml.tftpl` por `templatefile()` y al `runner-bootstrap.sh` por argumento (`RUNNER_COUNT=<n>` exportado en el `runcmd` o pasado al script por `sudo bash /opt/bootstrap-runner.sh "$RUNNER_COUNT"`).
...
| Area | Files | Change |
|---|---|---|
| Terraform | `infra/environments/dev/variables.tf` | nueva `variable "runner_count"` (default 2, validation 1..4) |
...
- **Contención CPU con la app.** N agentes corriendo jobs pesados (suite del backend, build multiarch) sobre una VM de 4 OCPU/24 GB puede degradar la latencia del frontend público. Mitigación: R6.3 exige una nota de medición (tiempo de suite del backend con otro agente corriendo la del frontend en paralelo + latencia p50 del frontend público) antes de aceptar `runner_count > 3`. La métrica se archiva en `RUNBOOK.md §6.2`.
...
- **Reporte de error por agente.** Cada iteración del bucle atrapa su propio error con nombre del agente (`agent <i>/<N>: <acción>: <error>`); un fallo en el agente `k` deja los agentes `1..k-1` reconciliados y los `k+1..N` sin tocar. El operador ve qué se hizo y qué no, y reaplica solo con la fix del error — no tiene que empezar de cero.

## Open questions

Ninguna que bloquee el `/sdd:tasks`. La decisión de `runner_count = 2` como default y el rango 1..4 son del operador y se revisan en cada reaprovisionamiento; la nota de medición para `runner_count > 3` (R6.3) es la única decisión que requiere datos del entorno desplegado, y se hace **antes** de aceptar el cambio, no durante.
