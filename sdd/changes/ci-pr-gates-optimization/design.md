...
| `compose-ports` | `make check-compose-ports` → `scripts/compose-ports.py`; **`pytest scripts/ -q`** (el árbol de tests de scripts **entero**, no solo compose) | `compose.yaml | compose.yml | compose.override.yaml | compose.override.yml | docker-compose*.yml | docker-compose*.yaml | docker-compose.worktree.yml | scripts/* (todo el árbol que `pytest scripts/` ejecuta) | Makefile (target) | workflow` |
