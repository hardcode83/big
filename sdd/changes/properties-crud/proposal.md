...
3. WHEN un `PATCH` no cambia nada —cuerpo vacío o campos con el valor que ya tenían— THE SYSTEM SHALL no escribir ni fila ni `AuditLog`: `audit_logs` es evidencia de cambios, no de peticiones (`user-management.md:119-121`).
   - **Excepción única y nombrada: `wifi_password`.** WHERE el cuerpo incluye `wifi_password`, THE SYSTEM SHALL tratarlo siempre como un cambio, aunque el valor sea idéntico al almacenado. No es una concesión sino una consecuencia inevitable de D1: el valor se guarda cifrado y **no existe lector**, así que no hay con qué compararlo — detectar el no-op exigiría descifrarlo, que es justo la operación que R5 prohíbe montar. Escribir de más es el lado seguro del error: la alternativa sería no reescribir una contraseña que el llamante pidió cambiar. Añadida el 2026-08-08 tras el panel de `/sdd:review`, que encontró la contradicción entre el texto literal de este criterio y el comportamiento entregado; el comportamiento ya estaba probado a propósito en `test_property_admin.py`.
