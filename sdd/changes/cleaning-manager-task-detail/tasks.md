...
## 6. i18n y navegación: namespaces `detail.*` y `routes.cleaning-detail.*` <!-- panel: PASS 2026-09-17 receipt:83d926ec -->

- [x] 6.1 `frontend/locales/es/cleaning.json` y `frontend/locales/en/cleaning.json` —
...
- [x] 6.2 `frontend/locales/es/navigation.json` y `frontend/locales/en/navigation.json` —
...
- Full-project typecheck (`docker compose exec -T frontend npx tsc --noEmit`) ahora 0 errores tras añadir `reservationId: null` a 6 fixtures de `CleaningTask`/`CleaningTaskListItem` (en `cleaning-task-row.test.tsx`, `cleaning-view.test.tsx`, y los 4 `hooks/use-*-cleaning-task.test.tsx`) y `getTask: vi.fn()` a 5 mocks de `CleaningDataSource` (en `cleaning-view.test.tsx`, `use-cleaning-data.test.tsx`, y 3 de los hooks); las 525/525 tests de `features/cleaning/` siguen verdes.
...
- Fix R6.1 round 1 (sdd-qa medium): añade las 7 entradas que la sección 6 había diferido al catálogo de `cleaning` (es y en): `detail.title`, `detail.manager.title`, `detail.assigned.label`, `detail.context.label`, `detail.identifying.propertyNotFound`, `detail.header.scheduledWindowValue`, `detail.error.unknown` — antes producción renderizaba la ruta literal. Tres aserciones de tests pasan de clave cruda a valor ES (`detail-assigned-cleaner-block.test.tsx`: "Limpiadora"; `detail-identifying-block.test.tsx`: "Vivienda no disponible"; `detail-manager-actions-block.test.tsx`: "Acciones del manager").
...
- F1 (sdd-review-documentation): nueva sección §«Detalle operativo del manager» en `docs/cleaning.md` (antes de «Entradas de roadmap relacionadas») que describe `/cleaning/[id]` — quién la ve (PROPERTY_MANAGER + TENANT_OWNER, mismo gate `AuthGuard` workspace), los seis bloques en orden (cabecera, identificador, limpiadora asignada, acciones del manager, enlaces de contexto) con la matriz «qué ve cada rol», cómo encaja con lo existente (backend intacto, region viva única, mobile-first), y lo que sigue sin ser (checklist/fotos siguen en la app de la limpiadora). Reescrito el texto obsoleto en `docs/cleaning.md:502-504` (sobre «no abre el detalle») para reflejar que la fila del listado enlaza a `/cleaning/[id]` y la pantalla pinta los seis bloques. Entrada nueva en «Entradas de roadmap relacionadas»: `cleaning-manager-task-detail — ya entregada: /cleaning/[id]».
- F2 (sdd-review-ui-ux): `cleaning-task-detail-view.tsx` rama `not-found` ahora usa `EmptyState` con `title` + `action` (sin `description`, que duplicaba el texto del enlace). El título dice «Tarea no disponible» y la action mantiene el `Link href="/cleaning"` con «Volver al listado». El test existente («renders the not-found state with a back link (R1.2)») sigue verde porque sigue habiendo un único enlace con `href="/cleaning"` y el título «Tarea no disponible».
- F3 (sdd-review-ui-ux): el `Link` en `cleaning-task-row.tsx:197` añadía `focus-visible:underline focus-visible:outline-none`, y el `outline-none` borraba el anillo de foco dejando sólo el underline (compartido con hover). Añadido `focus-visible:rounded-sm focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background` — anillo visible con los tokens `--ring`/`--background` que `frontend/components/ui/button.tsx:8` ya emplea para el resto de botones del proyecto.
- F4 (sdd-review-ui-ux): `CancelOpenButton` en `detail-manager-actions-block.tsx:142` usaba un `<button>` con estilos inline sin estado disabled visible (`AssignCleanerControl` y `ValidateCleaningControl` usan el `Button` de `@/components/ui/button`, que ya trae `disabled:pointer-events-none disabled:opacity-50`). Añadido `disabled:opacity-50 disabled:cursor-not-allowed` a las clases para que la convención del proyecto sea explícita también aquí. El test existente (`disables the cancel button while a cancellation is in flight`) sigue verde: usa `toBeDisabled()` (chequeo semántico del atributo `disabled`), insensible al cambio de clase.
...
- Catálogo `cleaning:detail.*` añade 21 claves (las 21 listadas en 6.1) en `es` y `en`; las claves que la sección 4 usa pero la tarea 6.1 no enumera (`detail.title`, `detail.error.unknown`, `detail.manager.title`, `detail.assigned.label`, `detail.context.label`, `detail.identifying.propertyNotFound`, `detail.header.scheduledWindowValue`) siguen en key-path fallback — los asserts de los tests para esas claves (`detail.manager.title`, `detail.assigned.label`, `detail.identifying.propertyNotFound`) siguen verdes sin tocarse.
- Tras llenar el catálogo, los asserts de los tests que vivían contra key paths (`detail.notFound`, `detail.forbidden`, `detail.validation`, `detail.error.title`, `detail.error.retry`, `detail.header.validation`, `detail.header.status/completedAt/validatedAt/scheduledWindow`, `detail.identifying.propertyCode/propertyName/reservationCode/reservationNotFound`, `detail.assigned.unassigned/notFound`, `detail.context.viewProperty/viewReservation/backToList`) se actualizan a las traducciones `es` — los tests corren con `I18nProvider locale="es"`, así que las constantes `*_LABEL` y los literales directos son los valores del catálogo español.
- `routes.cleaning-detail.{title,description}` insertado en `navigation.json` (`es` y `en`) entre `cleaning` y `incidents`, replicando la forma de `incident-detail`; el gate `routeRegistry resolves every route descriptor key` del `catalog-parity.test.ts` ya estaba en rojo antes de la sección (referencia a `routes.cleaning-detail.title` indefinida) y pasa tras añadir las claves.
- Verificaciones: `lib/i18n/catalog-parity.test.ts` 24/24 verde; `features/cleaning/**` completo 29 ficheros, 525 tests, ~5 s verde (mismas cifras que la sección 5; los tests modificados no añadieron tests, sólo cambiaron literales).
- `tsc --noEmit` no se ha ejecutado: el precedent de OOM (`cleaning-task-manage-web` 7.3) y la nota de sección 5 («errores preexistentes de las secciones 1-2») lo descartan; la suite vitest cubre el comportamiento runtime.
- Fix round 2 (review F1, R5.4 medium): `cleaning-task-detail-view.tsx announcement()` importa `assignErrorKey` de `lib/assign-error` y `keyForStatus`+`VALIDATE_ERROR_TABLE`+`GENERIC_VALIDATE_ERROR_KEY` de `lib/manage-error`; el `role="alert"` ahora pinta el mensaje diferenciado por status (403/404/409/422) en lugar de `detail.error.unknown`. La rama cancel sigue exenta (la maneja `CancelCleaningTaskDialog` per D7). Nuevo test en `cleaning-task-detail-view.test.tsx`: dispara un `ApiError(403)` en el mock de `useAssignCleaningTask` y verifica que el `role="alert"` contiene "No tienes permiso para asignar limpiezas" y NO contiene "Ha ocurrido un error inesperado".
- Fix round 2 (review F2, R3.1 low): `detail-identifying-block.tsx` ahora degrada `pending` a un guion + `sr-only` "Cargando identidad…" (vía `identity.loading`, mismo namespace que `detail-assigned-cleaner-block.tsx:52`), mientras que `unavailable` mantiene `detail.identifying.propertyNotFound`. Un refactor `renderPropertyField(field)` encapsula el switch — `unassigned` no aplica aquí (`propertyId` no es nunca `null` en `CleaningTask`), pero se mantiene la rama por exhaustividad del union. Nuevo test verifica que con `isPending: true` el bloque pinta "Cargando identidad…" en las dos celdas y NO pinta "Vivienda no disponible".
- Fix round 2 (review F3, R1.2 low): `cleaning-task-detail-view.test.tsx:172` ahora usa `expect(backLinks.length).toBe(1)`, fija el round-1 contra una regresión que reintroduzca `description={t("detail.context.backToList")}` (esa duplicación no sumaba anchors pero sí duplicaba texto visible).
- Fix round 2 (review F4, R6.2 low): `detail-manager-actions-block.test.tsx:132-138` añade `expect(button).toHaveClass("disabled:opacity-50")` y `expect(button).toHaveClass("disabled:cursor-not-allowed")` después del `toBeDisabled()` — el atributo semántico sigue cubierto pero ahora la apariencia CSS del estado disabled queda protegida contra una regresión que borre las clases del round 1.
- Fix round 2 (review F5, R2.3 low): `detail-header-block.tsx` ahora selecciona el label de `validatedAt` con `validationStatus === "FAILED"` → `detail.header.validatedAtFailed` ("Validación rechazada" / "Validation rejected"), cualquier otro veredicto mantiene `detail.header.validatedAt`. Claves nuevas en `locales/{es,en}/cleaning.json`. Nuevo test en `detail-header-block.test.tsx`: renderiza `validationStatus: "FAILED"` y verifica que "Validación rechazada" aparece y "Fecha de validación" no.
