...
- [x] N.1 Suite backend completa verde: `docker compose exec backend uv run pytest` — incluye `tests/platform/` (cuando exista), `tests/tenants/`, `tests/auth/`, `tests/cli/`, `tests/test_route_authorization.py`, `tests/test_openapi_contract.py`, `tests/test_layering.py`, `tests/test_unscoped_reads.py`. Comparar el total de ficheros y tests contra el `pytest --collect-only` de partida del worktree (no contra el del principal — `sdd/project.md` §Worktree bootstrap). [testing.md] — **VERIFICADO (`/sdd:review`)**: `10090 passed, 43 skipped, 0 failed` en 9:14 (`pytest -q -n 2`, contenedor `backend` parado para evitar contención). Total colectado 10133, idéntico al de la pasada previa (10089+1+43), así que ningún fichero se cayó de la colección. La pasada previa tuvo 1 `worker crashed` en `tests/cleaning/test_photos_api.py` que en aislado da `15 passed`: contención de host, no regresión.
- [x] N.2 `uv run pyright .` (con `uv sync --frozen` hecho dentro de `backend`) sin findings nuevos en `app/platform/`, `app/auth/domain/policy.py`, `app/tenants/domain/entities.py`, `app/audit/domain/actions.py`. Los findings preexistentes en módulos no tocados no se imputan a este change. [testing.md] — **VERIFICADO**: `pyright` da 906 errores en todo el repo y **0 en el ámbito de esta tarea**. Los dos de `app/tenants/infrastructure/repositories.py` (`rowcount` sobre `Result[Any]`) son preexistentes: están en HEAD en las líneas 72 y 197, y las líneas que este change toca son la 23 y la 105.
- [x] N.3 Aislamiento de tenant: ejecutar `tests/platform/test_isolation.py` y `tests/platform/test_authorization.py` con la suite global y verificar que (a) cada rol no-`SUPER_ADMIN` recibe `403` con un único motivo en ambas rutas, (b) un `tenant_id` inexistente o `SUSPENDED` recibe `404` indistinguible, (c) un email ya usado bajo otro tenant recibe `409`. [R5.3, R3.3, R3.4] — **VERIFICADO**: verdes dentro de la suite completa. El aislamiento no es vacuo: el actor `SUPER_ADMIN` va con `tenant_id=None`, `bind_session_to_tenant` no se llama y la petición entera corre sobre sesión NO marcada, así que las tres lecturas (`bajo A`, `bajo B`, `bajo NULL`) ven todas las filas y pueden fallar.
- [x] N.4 Concurrencia (R-2): dos corutinas lanzan `POST /api/v1/platform/tenants` con el mismo `name` contra la misma BD; una termina en `201` con la fila en `tenants`, la otra en `409` con `code=CONFLICT` y la BD queda con exactamente una fila para ese nombre y una sola fila `TENANT_CREATED` en `audit_logs`. [R1.2, R-2] — **VERIFICADO con test concurrente real**: `test_two_truly_concurrent_tenant_creations_with_the_same_name_end_in_one_201_and_one_409` (dos sesiones sobre `test_engine` vía el fixture `api_session_per_request` + `asyncio.gather`). Determinista 5/5, y probado por mutación: desalineando el nombre de la restricción, falla con `IntegrityError` sin traducir. El duplicado salta en el `flush` del repositorio —no en el commit— porque Postgres bloquea el segundo INSERT sobre la entrada de índice sin commitear del primero.
- [x] N.5 Re-ejecución del bootstrap (R7.3): ejecutar `python -m app.cli.bootstrap` con `.env` apuntando a la BD del worktree; una segunda ejecución con el mismo `.env` debe terminar con `tenants=0, tenant_configs=0, users=0` (o `tenant_configs_converged=1` si `BOOTSTRAP_STORAGE_TYPE` cambia); el alta de un tercer tenant por la API no afecta a una segunda ejecución del bootstrap. [R7.1, R7.2, R7.3] — **VERIFICADO por CLI**: `python -m app.cli.bootstrap` ×3 contra la BD del worktree. Run 1: `1 tenant, 1 config, 3 users`. Run 2: `0/0/0`. Alta de un tercer tenant **por la API real** en medio (`201`), y run 3: `0/0/0`. Requirió rellenar las `BOOTSTRAP_*` del `.env` local (venían vacías de la plantilla y el CLI se niega a arrancar sin ellas, por diseño). De paso, `alembic upgrade head` aplicó la cadena unificada sobre BD real atravesando las dos merge revisions hasta `4ba1f499f7c2`.
- [x] N.6 Confirmar que `tests/test_route_authorization.py` queda verde después del cambio del snapshot (5.1) y del nuevo test estructural (5.2): cada ruta nueva declara `MANAGE_PLATFORM` y vive bajo `/api/v1/platform/`; el snapshot protegido incluye las dos rutas. [R6.1, R6.2, R-6] — **VERIFICADO**: verde dentro de la suite completa, con las dos rutas nuevas en el snapshot protegido y el test estructural (que fija la FORMA —prefijo `/api/v1/platform/`— y recorre todas las rutas registradas, no una lista de nombres mantenida a mano).
- [x] N.7 Confirmar que la guardia de la regla 11 no se ve afectada. **La tarea nombraba `tests/test_rule11_ownership.py`, que no existe**: la guardia es `make check-rule11-ownership` (Makefile:375), implementada por `scripts/rule11-ownership.py`, con sus propios tests en `scripts/test_rule11_ownership.py` y su workflow `.github/workflows/rule11-ownership.yml`. Correr el target, no buscar el fichero. Este change no introduce nuevas columnas con texto en claro (las cinco columnas del cuerpo de `POST /tenants` ya estaban cubiertas por el censo de `TenantConfigPatch`/`tenant_configs`; el `full_name` y `email` del alta de usuario son campos USER ya cubiertos). Si el guard encuentra algo, es un falso positivo del propio guard o un cambio accidental. [security.md regla 11] — **VERIFICADO** con el target real: `make check-rule11-ownership` → «ningún bloque fuera de la tabla de la regla 11 declara quién escribe un sumidero del censo» (102 markdown y 886 python recorridos).
- [x] N.8 `backend/openapi.json` regenerado y commiteado junto al `platform-router` y al `platform-error-handlers`; `cd frontend && npm run api:check` con el workaround de ENOENT documentado en `sdd/project.md` §Worktree bootstrap (no aplica en CI). El `.d.ts` generado incluye el namespace `platform` con los dos DTOs. [R6.3, project.md §Worktree bootstrap] — **VERIFICADO**: `backend/openapi.json` regenerado y commiteado (99 rutas, los dos conjuntos tras absorber `main`); `npm run api:check` → «generated types are up to date»; el `.d.ts` trae `/api/v1/platform/tenants`, `/api/v1/platform/tenants/{tenant_id}/users`, `CreateTenantRequest`, `CreatePlatformUserRequest` y `CreatedPlatformUserResponse`.
...
- **`/sdd:review` (pase de panel completo)**: la doble restricción única que la nota de
  arriba describía queda resuelta, y no como proponía: quitar `unique=True` habría dejado
  la BD de test —que se construye con `create_all` desde el metadata, no con migraciones—
  sin restricción alguna, y los 409 de R1.2 habrían dejado de poder fallar. Lo que se hizo
  es declarar `UniqueConstraint('name', name='uq_tenants_name')` en `__table_args__`: los
  dos caminos de esquema producen ahora el MISMO nombre, `tenants_name_key` ya no se crea,
  y `_UNIQUE_NAME_CONSTRAINTS` (frozenset de dos grafías) es ahora `_UNIQUE_NAME_CONSTRAINT`,
  una sola. Que los tests de duplicado sigan verdes es la prueba de que el nombre casa: si
  no casara, el `IntegrityError` se re-lanzaría sin traducir y darían 500.
- **`/sdd:review`**: el UNIQUE nuevo rompió dos tests ajenos al change
  (`tests/notifications/test_escalate_slas_atomicity.py`), que sembraban dos tenants con el
  nombre fijo `"TenantA"` en el mismo test. Semilla uniquificada con `uuid4`, como ya hacía
  el email de ese mismo helper.
- **`/sdd:review`**: `CreatePlatformUserRequest` aceptaba `full_name` vacío y `phone` sin
  cota, mientras `CreateUserRequest` —el cuerpo que su propio docstring dice replicar, y que
  R4.3 manda reutilizar— exige `min_length=1` y `max_length=MAX_PHONE`. En el mismo fichero,
  43 líneas antes, `CreateTenantRequest.name` ya llevaba `min_length=1` con el motivo escrito.
  Corregido; lo destapó el test de R3.6 que no existía.
- **`/sdd:review`**: N.4 tiene ya un test concurrente de verdad
  (`test_two_truly_concurrent_tenant_creations...`, dos sesiones sobre `test_engine` +
  `asyncio.gather`, con el fixture `api_session_per_request` en el conftest del paquete).
  Es determinista porque Postgres bloquea el segundo INSERT sobre la entrada de índice sin
  commitear del primero, así que el duplicado salta en el `flush` del repositorio —donde vive
  la traducción a 409— y nunca en el commit. Verificado 5/5 y con mutación: al desalinear el
  nombre de la restricción el test falla con `IntegrityError` sin traducir.
