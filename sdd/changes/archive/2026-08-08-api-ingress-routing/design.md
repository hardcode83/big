...
El /32 y no la subred entera, y el motivo es **medido** (reparto real de IPs en el deploy del 2026-08-08): `private` contiene `postgres 10.89.0.2`, `backend 10.89.0.5` y `frontend 10.89.0.10`, además de `worker`, `beat` y `migrate`. Confiar en la subred sería confiar en **seis servicios** para reportar la dirección de un cliente, cuando solo uno es el proxy. El /32 nombra exactamente al único que lo es.

**Y esta justificación se reescribió** (2026-08-08, al cerrar la sección 6): la primera decía que al publicar `127.0.0.1:8000:8000` Docker hace SNAT al gateway del bridge, que caería dentro de la subred. Es la **misma afirmación no medida** que el panel de seguridad hizo corregir en D7 — sobrevivió aquí porque la corrección aterrizó en una decisión y no en la otra, que es exactamente el modo de fallo del que este change lleva avisando. El razonamiento de arriba no necesita saber cómo enmascara Docker.
