...
| Entorno | `docker-compose.yml` | Monta **dos** ficheros del árbol como sólo-lectura en `backend`, cada uno con su propia procedencia. **(a) `./.env.example:/workspace/.env.example:ro`** — añadido en `/sdd:run`, **aprobado en el gate de la sección 2 (2026-08-23)**, y no estaba en este diseño: D18.2 exige un test que **lea** `.env.example`, y el contenedor monta sólo `./backend` como `/app`, así que sin el montaje el test sólo podría correr en CI. `backend/tests/auth/test_bootstrap.py` había declinado escribir ese mismo test por exactamente este motivo —«un test que no puede correr donde corre la suite es peor que ninguno»—, así que aceptar el montaje **revierte esa decisión a conciencia** y su docstring se corrige en el mismo change. **(b) `./.github/workflows/demo-reset.yml:/workspace/demo-reset.yml:ro`** — **no pasó por ningún gate**: lo levantó el panel de `/sdd:review` (2026-08-24) al encontrarlo sin registrar aquí, y se documenta a posteriori. Nada de lo dicho en (a) le aplica —ni la fecha del gate, ni D18.2, ni el episodio de `test_bootstrap.py`—; lo que lo justifica es otra cosa: la costura CLI↔workflow de D13 (el enlace llega al resumen del job por un `grep` de stdout) sólo se puede fijar con un test que **lea** el `.yml`, y el precedente exacto es el montaje de `deploy-dev.yml` que ya estaba en ese bloque por la regresión de provenance. Y lo que sí vale para los dos, por ser propiedades del montaje y no de su historia: siguen los precedentes ya presentes en ese bloque `volumes` (`./sdd`, `./docs`, `deploy-dev.yml`), son de sólo lectura, son dev-only —`docker-compose.deploy.yml` no los recibe— y no tocan la guardia de postura de red, que lee únicamente la clave `ports:`. |
...
| Tests | `backend/tests/cli/test_demo_reset.py` **(nuevo)**, `backend/tests/cli/test_seed_demo.py`, `backend/tests/auth/test_bootstrap.py` | D18 y la cobertura de la fase nueva del seed. |
