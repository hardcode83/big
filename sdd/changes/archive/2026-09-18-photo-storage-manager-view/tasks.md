...
- Review-fix round 1 (section 4 `sdd-review-ui-ux` FAIL, steering `frontend.md` "Testing UI/UX"): added a keyboard-reachability test (focus the error state's retry button, assert `tabIndex >= 0` and `toHaveFocus()`, matching the `.focus()`/`toHaveFocus()` convention already used in `tenant-settings`/`statements`) and a responsive/no-clipping test (assert the gallery's `<ul>` keeps `grid-cols-2` and each `<img>` keeps `w-full` — both fraction-based, so the layout can't overflow at narrow widths — matching the class-assertion convention in `statement-downloads.test.tsx`/`properties-view.test.tsx`, since jsdom does no real layout) to both `incident-photos-block.test.tsx` and its sibling `detail-photos-block.test.tsx` (section 3, same structural gap, fixed proactively). No component `.tsx` changes: both galleries already used a fraction-based grid with no fixed-width overflow risk — this was a test-coverage gap only.
- Review-fix round 1 (feature-scale `sdd-security` FAIL, `backend/app/cli/demo_reset.py:255`): `demo_reset.build_plan()` constructs `BootstrapPlan` directly (by design, D3) and so never called `bootstrap.build_plan()`'s R3.1/D6 guard — a manual `python -m app.cli.demo_reset` in dev/staging/production with no explicit `BOOTSTRAP_STORAGE_TYPE=S3` still converged the demo tenant to `LOCAL`. Fixed by extracting the raw condition into a shared, pure function, `reject_local_storage_outside_local_environment(environment, storage_type, *, exception_type, message)` in `backend/app/cli/bootstrap.py` (a caller-supplied exception type, so `bootstrap.py` still raises `BootstrapConfigurationError` and `demo_reset.py` its own `DemoResetConfigurationError`). Both `bootstrap.build_plan()` and `demo_reset.build_plan()` now call it, each in its own existing position, before constructing their `BootstrapPlan`. `backend/tests/auth/test_bootstrap.py` unchanged and green; `backend/tests/cli/test_demo_reset.py` got the equivalent three cases. `sdd/changes/photo-storage-manager-view/design.md`'s D6 got an addendum documenting the extraction and the second call site.
