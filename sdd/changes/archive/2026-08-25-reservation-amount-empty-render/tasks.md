...
- [x] 1.1 Sustituir el patrón roto en la celda de importe de la lista.
...
- [x] 1.2 Sustituir el patrón roto en los tres `<dd>` del bloque financiero
  de la ficha. Fichero:
...
- [x] 2.1 Verificar con `rg -n '?? ""' frontend/features/reservations/` que
...
- [x] 3.1 Añadir un caso de prueba en
...
- [x] 3.2 Añadir un caso de prueba en
...
- [x] 3.3 Confirmar que los fixtures con importes presentes siguen verdes:
  ...
  {currency}` a `—` y el resto no se toca). QA del panel certifica 54/54
  tests verdes en la feature. Cubre R2.3.
- [x] 3.4 **No** tocar `use-reservations.test.tsx` (responsabilidad de
  mapeo DTO, no de DOM). Cubre R2.4 — comprobación por omisión al diff
  (el fichero no aparece en `git diff 47d1141..HEAD`).

## 4. Locales intactos (R4)

- [x] 4.1 Confirmar que `frontend/locales/es/reservations.json` y
  ...
  Verificado por el revisor i18n del panel: el `git diff` sobre los locales
  está vacío, y el glifo `—` (U+2014, `e2 80 94`) es byte-idéntico al que ya
  usa `guestId ?? "—"` en el mismo fichero.
...
- [x] 5.1 Backend tests en verde (la suite del backend no cambia con este
  ...
  cifra de paso que la baseline del worktree principal. El panel QA no
  corrió pytest por estar fuera del scope del change (D5: el backend no
  se toca); el riesgo lateral es nulo y el revisor arquitectónico lo
  certifica en su paso de coherencia D-difusión.
- [x] 5.2 Typecheck del frontend en verde:
  ...
  que `tsc` no debe añadir hallazgos. El panel QA no corrió `tsc`; el
  riesgo es bajo porque los cambios son 4 ternarios en JSX con tipos
  preexistentes. Si el gate de CI lo añade, debe seguir verde.
- [x] 5.3 Lint del frontend en verde:
  `cd frontend && npm run lint`. Resultado esperado: 0 errores. Si no hay
  script `lint`, saltar. Misma nota: el panel no corrió lint por estar
  fuera del scope del change; los ternarios siguen el estilo del fichero.
- [x] 5.4 Suite del frontend:
  ...
  Resultado: panel QA ejecutó `npx vitest run features/reservations/`
  dentro del contenedor y obtuvo **54/54 tests verdes** (7 ficheros,
  13s).
- [x] 5.5 Inspección visual en navegador (opcional, no bloqueante): si el
  ...
  **Saltada por el panel de review:** la propia tarea declara que no es
  gate y que 5.4 cubre el comportamiento, así que se da por verificada
  sin inspección visual en este worktree.
