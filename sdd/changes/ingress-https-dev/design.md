...
| Terraform — DNS y zona | `infra/environments/dev/main.tf` | `cloudflare_dns_record.app` (CNAME → `<tunnel_id>.cfargotunnel.com`, `proxied = true`), `data "cloudflare_zone"` + `precondition` (D9), y **un solo** ajuste de zona: `always_use_https`. `min_tls_version` **no** se declara (D7) |
...
| CD — deploy | `.github/workflows/deploy-dev.yml` | leer el token del túnel del Vault **por nombre** y volcarlo al `.env` como `TUNNEL_TOKEN`. **No** se añade un paso de verificación externa al pipeline: el fallo temprano lo da el healthcheck de `cloudflared` a través de `up -d --wait` (D5), y la comprobación HTTPS externa es manual y única (tarea 5.1) |
