...
- Review fix (round 1, post-run): extended sdd/steering/security.md's dev/test tfstate exception paragraph (~line 52) to name the four whatsapp_* secrets alongside GH_APP_PRIVATE_KEY, per sdd-security's FAIL finding on /sdd:review.
- Review fix (round 2, post-run, opus): documented the four whatsapp_* GitHub Secrets in infra/environments/dev/README.md's "Despliegue de la app (CD — `app-deploy-dev`)" table (the one holding `GH_APP_PRIVATE_KEY`, not the `infra-dev` OCI/SSH table), per sdd-review-documentation's FAIL finding on the re-review after round 1. Same row also adds `WHATSAPP_PROVIDER` (repo **variable**, consumed by `deploy-dev.yml` itself). Two consequential corrections in that same block: the preamble's "No hay secrets de app de runtime en GitHub" now carves out externally-issued credentials (it was flatly contradicted by the new row), and `GH_APP_PRIVATE_KEY`'s "Único secret-zero" dropped the "Único" (the four Meta credentials are secret-zero by the same definition).
- Follow-up to round 2 (same turn, orchestrator): `infra/environments/dev/RUNBOOK.md` §6.1 had the same "único secret-zero" claim for `GH_APP_PRIVATE_KEY`, now stale for the same reason — corrected to "secret-zero del runner" with a pointer to README's table, no invented section reference.
