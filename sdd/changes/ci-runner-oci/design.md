...
- **Riesgo (aceptado, alcance ampliado en la tercera pasada de `/sdd:review`): mover los jobs disparados por `pull_request` al mismo runner persistente que ejecuta `terraform apply`, el push a GHCR y el deploy real amplía el radio de confianza en tres formas distintas, no solo una:**
  ...
  2. **Lectura directa del `.env` de dev en el workspace compartido.** El job `deploy` renderiza el `.env` real (con los mismos secretos de arriba en texto plano) al workspace del runner, que es físico y compartido entre workflows (`demo-reset.yml` usa `clean: false` justo porque lo comparte con `deploy` — es su forma deliberada de *consumir* ese `.env`, con una precondición explícita que falla si no está: `demo-reset.yml` "Precondición — el entorno está desplegado"). Un PR que ponga `clean: false` en su propio `actions/checkout` podría leer el mismo `.env` que `demo-reset` está diseñado para leer — sin pasar por IAM ni por Vault. **Intentado y revertido en la tercera pasada**: un `rm -f .env` con `if: always()` al final de `deploy` rompía exactamente el mecanismo que `demo-reset.yml` necesita (su precondición y la interpolación de `docker-compose.deploy.yml` dependen de que ese fichero sobreviva) — hallazgo del panel de seguridad, cuarta pasada, revertido en el mismo commit. **Sin mitigación técnica en este change**: arreglarlo de verdad exige mover el `.env` fuera del workspace compartido (p. ej. a `$RUNNER_TEMP`, con `--env-file` explícito en ambos jobs) y tocar `demo-reset.yml` más allá de la cabecera — fuera del alcance de "solo cambia el runner" que R2 fija para este change. Queda como riesgo aceptado, como (3).
  3. **Control root-equivalente sobre el despliegue de dev en vivo, no solo lectura de secretos.** El usuario del runner está en el grupo `docker` (root-equivalent vía el socket) en el mismo host que ejecuta el stack de dev desplegado y público: los contenedores de backend/worker/frontend, el volumen de datos de Postgres, y `cloudflared`, que publica ese host en **https://autohostai.digitalsec.work**. Código de PR con acceso al socket de Docker puede tamperar con esos contenedores o el volumen, no solo leer secretos — esto es un salto de impacto mayor que "leer un secret-bundle", y no estaba nombrado explícitamente en la primera versión de esta nota.
  ...
  Mitigación: **riesgo aceptado explícitamente por el usuario, alcance completo confirmado** (revisión 2026-09-03 — ronda 1 aceptó (1); ronda 3 nombró (2) y (3) explícitamente, incluyendo el control root-equivalente sobre el deploy público, y el usuario confirmó mantener la aceptación tal cual, sin segregar runners ni tocar el grupo `docker` del usuario del runner de PR) — el repo es privado, los colaboradores con permiso de abrir PR son de confianza, y no se justifica hoy un segundo runner o una política IAM/Docker más restrictiva solo para esta migración (ver Open question 2, ya cerrada como "no en este change"). Ni (2) ni (3) tienen mitigación técnica en este change — ambas son inherentes a que el deploy real y los jobs de PR comparten runner, y quedan como riesgo aceptado, no como pendiente. Reevaluar el conjunto si (a) se añade un secreto nuevo a la enumeración de `main.tf:258`, (b) el repo pasa a aceptar colaboradores externos con permiso de abrir PR, o (c) el coste de un segundo runner/pool dedicado a jobs `pull_request` deja de ser desproporcionado — o de una migración de `.env` a `$RUNNER_TEMP` con `--env-file` explícito, para (2) en concreto.
