...
- **Riesgo (aceptado, alcance ampliado en la tercera pasada de `/sdd:review`): mover los jobs disparados por `pull_request` al mismo runner persistente que ejecuta `terraform apply`, el push a GHCR y el deploy real amplía el radio de confianza en tres formas distintas, no solo una:**
  ...
  Mitigación: **riesgo aceptado explícitamente por el usuario** (revisión 2026-09-03, ronda 1: (1); ronda 3, con el alcance completo de (2) y (3) ya nombrado: pendiente de confirmación explícita del usuario sobre esta versión ampliada) — el repo es privado, los colaboradores con permiso de abrir PR son de confianza, y no se justifica hoy un segundo runner o una política IAM/Docker más restrictiva solo para esta migración (ver Open question 2, ya cerrada como "no en este change"). (2) tiene mitigación técnica aplicada (borrado del `.env` al final del job). (3) no tiene mitigación técnica en este change — es inherente a que el deploy real y los jobs de PR comparten runner, y solo se cierra segregando runners o quitando `docker` del usuario del runner de PR, ninguna de las cuales se adopta aquí. Reevaluar el conjunto si (a) se añade un secreto nuevo a la enumeración de `main.tf:258`, (b) el repo pasa a aceptar colaboradores externos con permiso de abrir PR, o (c) el coste de un segundo runner/pool dedicado a jobs `pull_request` deja de ser desproporcionado.
