...
- **Riesgo (aceptado, alcance ampliado en la tercera pasada de `/sdd:review`): mover los jobs disparados por `pull_request` al mismo runner persistente que ejecuta `terraform apply`, el push a GHCR y el deploy real amplía el radio de confianza en tres formas distintas, no solo una:**
  ...
  2. **Lectura directa del `.env` de dev en el workspace compartido.** El job `deploy` renderiza el `.env` real (con los mismos secretos de arriba en texto plano) al workspace del runner, que es físico y compartido entre workflows (`demo-reset.yml` usa `clean: false` justo porque lo comparte con `deploy`). Un PR que ponga `clean: false` en su propio `actions/checkout` podría leer un `.env` que quedara sin borrar de un `deploy` anterior — sin pasar por IAM ni por Vault. **Mitigado en esta misma ronda**: `deploy-dev.yml` ahora borra `.env` con `if: always()` al final del job (paso "Borrar .env del workspace"), así que no debería sobrevivir a un job completo — pero el borrado depende de que el job termine (no cubre un runner matado a mitad, mismo matiz que el riesgo del `OCI_PRIVATE_KEY` de arriba).
