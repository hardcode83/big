...
4. **(Enmienda 2026-09-03, sexta pasada de `sdd-security`, con el usuario)** `deploy-dev.yml`/`demo-reset.yml` reciben un cambio adicional, más allá de `runs-on` + cabecera: el `.env` de `deploy` se mueve del workspace del checkout a `$HOME/.autohostai-dev-runtime.env`, con `--env-file` explícito en los 8 `docker compose -f docker-compose.deploy.yml` de ambos ficheros, y `demo-reset.yml` vuelve al checkout por defecto. Motivo: con 7 workflows `pull_request`-triggered más compartiendo el mismo workspace físico, el `git clean -ffdx` del checkout por defecto de cualquiera de ellos borraba el `.env` que `demo-reset.yml` necesita — una rotura real de la migración, no un riesgo hipotético (detalle en Risks & mitigations). Autorizado explícitamente por el usuario como excepción a "solo `runs-on` + cabecera" para estos dos ficheros.
5. **(Enmienda 2026-09-03, sexta pasada de `sdd-security`)** `backend-tests.yml`: los puertos de los `services` (`postgres`/`redis`) pasan de `5432:5432`/`6379:6379` a `127.0.0.1:5432:5432`/`127.0.0.1:6379:6379`. Motivo: en `ubuntu-latest` publicar a `0.0.0.0` no importaba (VM efímera sin IP alcanzable); en el runner persistente con IP pública real es una exposición innecesaria — nada fuera del propio job necesita alcanzar Postgres/Redis. Tercera excepción, no enumerada hasta ahora, a "solo `runs-on` + cabecera".
6. **(Enmienda 2026-09-03, cuarta pasada de `sdd-security`)** `deploy-dev.yml`: `build-backend`/`build-frontend` ganan un paso `docker logout ghcr.io` (`if: always()`), igual que ya tenía `deploy`. Motivo: la credencial de GHCR no debe sobrevivir al job en el runner persistente. Cuarta excepción.

Rejected: tocar `actions/checkout` con `clean: false` por defecto — ningún workflow lo necesita ya (ver enmienda 4 arriba: `demo-reset.yml` volvió al checkout por defecto en cuanto el `.env` dejó de vivir en el workspace). Cambiar el default contaminaría el árbol del runner con artefactos de un job en otro.
...
**Chosen:** el workflow se migra a `[self-hosted, dev]` con la misma configuración de hoy (`docker/setup-qemu-action@v3` + `docker/setup-buildx-action@v3` + `docker/build-push-action@v6` con `platforms: linux/amd64,linux/arm64`, `push: false`). La acción `docker/setup-qemu-action@v3` registra `binfmt_misc` desde una imagen Docker; en runners self-hosted requiere que el usuario del runner tenga acceso a `/proc/sys/fs/binfmt_misc` (el usuario `ubuntu` con grupo `docker` lo tiene). Si la verificación falla en CI (R7), se revierte solo este workflow a `ubuntu-latest` y el runbook declara la excepción como "9 de 10 migrados, 1 excepción deliberada". **Nota (2026-09-03, `sdd-review-cicd` cuarta pasada):** en la práctica la verificación de R7 nunca llegó a ejecutarse — el probe de §1.2 quedó bloqueado (VM IP no reachable desde el worktree) y se adoptó el fallback de R4.2/D5 directamente, así que la excepción documentada en el runbook/README/`r7-evidence.md`/`STATE.md` es **"QEMU no verificado — probe no ejecutado, no un fallo confirmado"**, no la razón técnica específica que este párrafo sugería (`QEMU no soportado...`) — esa frase describía el resultado que se *esperaba* documentar si el probe llegaba a correr y fallaba, no lo que ocurrió.
...
- **Riesgo: el workspace del runner (`/opt/actions-runner/_work`) acumula artefactos y crece el disco.**
  Mitigación: `actions/checkout` por defecto limpia el árbol del job en **todos** los workflows (`demo-reset.yml` incluido, desde la enmienda 4 de D3 — ya no necesita `clean: false`, ver Risks & mitigations). Se documenta en el runbook (D8 §6) que un rebuild de la VM recrea el runner con workspace vacío (`oci-source-id-update-replaces-boot-volume` ya advierte de que tocar la VM viva es delicado).
...
- **Riesgo: el `OCI_PRIVATE_KEY` queda escrito a `$RUNNER_TEMP` y un job abortado deja el fichero hasta el siguiente cleanup del runner.**
  Mitigación: `umask 077` precede al `printf` en `infra-dev.yml` (`plan` y `apply`), así el fichero nace `600` en vez de heredar el umask por defecto del proceso — corregido tras el panel de seguridad (`ci-runner-oci` review, 2026-09-03), que encontró el `printf` original sin ninguna restricción de modo. **Corregido (sexta pasada, `sdd-security`):** `$RUNNER_TEMP` NO se limpia al terminar el job — se limpia cuando el *siguiente* job inicializa, así que en el runner persistente el fichero sobrevive entre el fin de este job y el arranque del próximo. Por eso el paso "Limpiar ficheros con estado/credenciales del workspace" (`if: always()`, mismo paso que borra `errored.tfstate`) ahora borra también `$RUNNER_TEMP/oci_private_key.pem` explícitamente, en vez de confiar en el cleanup automático. El runbook (D8 §4) advierte de que un kill -9 del runner puede dejar el fichero unos segundos.

- **Riesgo (aceptado, alcance ampliado en la tercera pasada de `/sdd:review`): mover los jobs disparados por `pull_request` al mismo runner persistente que ejecuta `terraform apply`, el push a GHCR y el deploy real amplía el radio de confianza en dos formas distintas:**
  1. **Lectura de secret-bundles vía IAM.** El instance principal del runner puede leer, vía `read secret-bundles ... where any {target.secret.id = ...}` (`infra/environments/dev/main.tf:258`, `iam-policy.md`), los 16 secret-bundles enumerados explícitamente en esa policy — no el compartment entero: un secreto nuevo es invisible al runner hasta añadirse a la enumeración (`iam-policy.md:98`). De esos 16, los que convierten esto en escalada real (no solo lectura de config) son `oci_vault_secret.github_app_key` (clave privada de la GitHub App con `administration: write`, `design.md:5`), `jwt_secret_key` (forja JWTs de sesión) y `encryption_key` (descifra PII protegida con Fernet, `steering/security.md` regla 3).
  2. **Control root-equivalente sobre el despliegue de dev en vivo, no solo lectura de secretos.** El usuario del runner está en el grupo `docker` (root-equivalent vía el socket) en el mismo host que ejecuta el stack de dev desplegado y público: los contenedores de backend/worker/frontend, el volumen de datos de Postgres, y `cloudflared`, que publica ese host en **https://autohostai.digitalsec.work**. Código de PR con acceso al socket de Docker puede tamperar con esos contenedores o el volumen, no solo leer secretos — esto es un salto de impacto mayor que "leer un secret-bundle", y no estaba nombrado explícitamente en la primera versión de esta nota.
  Antes de este change esos jobs corrían en un runner GH-hosted efímero sin ninguna credencial de OCI y sin compartir host con el deploy real. `sin acceso a secretos fuera del workspace del job` (versión original de esta nota, ronda 1) era una premisa no verificada y la contradecía `iam-policy.md`.
  Mitigación: **riesgo aceptado explícitamente por el usuario, alcance completo confirmado** (revisión 2026-09-03 — ronda 1 aceptó (1); ronda 3 nombró (1)/(2)/una tercera superficie explícitamente, incluyendo el control root-equivalente sobre el deploy público, y el usuario confirmó mantener la aceptación tal cual, sin segregar runners ni tocar el grupo `docker` del usuario del runner de PR) — el repo es privado, los colaboradores con permiso de abrir PR son de confianza, y no se justifica hoy un segundo runner o una política IAM/Docker más restrictiva solo para esta migración (ver Open question 2, ya cerrada como "no en este change"). (2) no tiene mitigación técnica en este change — es inherente a que el deploy real y los jobs de PR comparten runner, y queda como riesgo aceptado, no como pendiente. Reevaluar el conjunto si (a) se añade un secreto nuevo a la enumeración de `main.tf:258`, (b) el repo pasa a aceptar colaboradores externos con permiso de abrir PR, o (c) el coste de un segundo runner/pool dedicado a jobs `pull_request` deja de ser desproporcionado.

- **Riesgo (era la tercera superficie de la nota anterior — la rotura funcional está arreglada, la superficie de lectura NO está mitigada, está desplazada; ver historial en `tasks.md` §9 entradas 15, 19 y 21): lectura directa del `.env` de dev, y antes de esto, rotura funcional del mismo mecanismo por actividad de CI normal.** El job `deploy` renderiza el `.env` real (secretos en texto plano) y `demo-reset.yml` lo consumía leyéndolo del mismo workspace físico compartido con `clean: false`. Desde ci-runner-oci, 7 workflows `pull_request`-triggered más comparten ese workspace; su `actions/checkout` **por defecto** (`clean: true`) borra ficheros no versionados como `.env` — así que el primer PR normal disparado tras cualquier deploy real ya lo borraba, sin que hiciera falta ningún PR malicioso, y la precondición de `demo-reset.yml` habría fallado en su siguiente ejecución de las 03:15 (hallazgo del panel de seguridad, sexta pasada — una regresión funcional real de este change, no solo una superficie de seguridad).
  Mitigación (**aplicada, sexta pasada, con el usuario — solo para la rotura funcional**): el `.env` de `deploy` se renderiza ahora en `$HOME/.autohostai-dev-runtime.env` — fuera de `$GITHUB_WORKSPACE` y fuera de `$RUNNER_TEMP` (que también se limpia entre jobs), así que ningún checkout de ningún workflow lo toca y `demo-reset.yml` vuelve a funcionar sin `clean: false`. **Efecto secundario, corregido en la séptima pasada (`sdd-security`): la superficie de lectura del `.env` no se redujo, se alargó en el tiempo.** Antes vivía en el workspace solo hasta el próximo checkout con `clean: true` (ventana acotada); en `$HOME` no hay ningún mecanismo de Actions que lo borre nunca — persiste indefinidamente, legible por el usuario del runner desde cualquiera de los 9 workflows migrados. Esto es la misma superficie que el riesgo (2) de la nota anterior (jobs `pull_request` compartiendo host con credenciales de deploy), ahora con un canal más directo (leer un fichero, sin pasar por IAM/Vault) — **aceptado como parte del mismo riesgo ya aceptado por el usuario**, no una mitigación nueva ni pendiente. Los 8 `docker compose -f docker-compose.deploy.yml` de `deploy-dev.yml`/`demo-reset.yml` pasan `--env-file "$HOME/.autohostai-dev-runtime.env"` explícito en vez de depender del autoload de Compose sobre el directorio del proyecto.

- **Riesgo (aceptado): `deploy-dev` (`build-backend`, `build-frontend`, `deploy`), `infra-dev` (`plan`, `apply`) e `infra-dev` (`check`) no tienen evidencia runner-side real antes de mergear.** `deploy-dev`/`infra-dev plan`/`infra-dev apply` llevan `if: github.ref == 'refs/heads/main'`, así que un `workflow_dispatch` sobre la rama del PR no los ejecuta — la evidencia R7 (`r7-evidence.md`) los registra `skipped`, no `success`. `infra-dev/check` es distinto: sí dispara por `pull_request`, pero su `paths: ["infra/environments/dev/**"]` (`infra-dev.yml:8-9`) no matchea el diff de esta migración (`.github/workflows/**`, docs, `backend/.python-version`), así que tampoco se dispara en este PR concreto — GitHub ni siquiera lo lista como check pendiente. La primera ejecución runner-side real de `docker buildx`, el push a GHCR, el deploy sobre la VM viva y `terraform apply` ocurre recién con el primer push a `main` tras el merge; la de `check` ocurre en el primer PR futuro que sí toque `infra/environments/dev/**` — ninguna de las dos "antes de merge" como enmarca R7.1.
  Mitigación: **riesgo aceptado explícitamente** (revisión con el usuario, 2026-09-03) — forzar esos jobs ahora exigiría un `terraform apply`/deploy real contra la VM dev fuera del flujo normal de merge, un coste mayor que el que motiva este change. **Matiz sobre `check`** (corregido 2026-09-03, `sdd-security` cuarta pasada): que no lea `secrets.*` (`init -backend=false`) acota su superficie *IAM/Vault*, no su riesgo runner-side — sigue siendo el único job `pull_request`-triggered de `infra-dev.yml`, ejecuta `terraform init`/`validate` con el código y los providers que el PR declara, como el usuario del runner (instance-principal con lectura ambiente de los 16 secret-bundles de `main.tf:258` y grupo `docker`); su riesgo real es el mismo riesgo de contorno de confianza ya nombrado arriba (jobs `pull_request` en el runner persistente), no uno "mínimo" aparte. En su lugar, `/sdd:ship` trata el primer run de `deploy-dev` y de `infra-dev apply` sobre `main` tras el merge, y el primer `infra-dev/check` del próximo PR que toque `infra/environments/dev/**`, como gates vigilados explícitos: si fallan en el runner por algo que `ubuntu-latest` no habría revelado (mismo patrón que D11), el rollback documentado en `docs/ci-runner-rollback.md` se ejecuta de inmediato.
