...
1. **Done 2026-09-03** (runs 33724393674 → fallo por el permiso IAM `manage credentials`, no anticipado — documentado en `iam-policy.md` — y 33725758274 en verde tras aplicarlo el admin). SPF verificado por `dig` (`v=spf1 include:eu.rp.oracleemaildelivery.com ~all`), DKIM CNAME resolviendo (`smtp-dev._domainkey.mail...` → `...dkim.fra1.oracleemaildelivery.com`), los seis secretos creados en el apply sin error.
2. **Done 2026-09-03** (run 33726191887 en verde; el auto-disparado por el merge, 33723337833, falló ANTES del apply de Terraform exactamente con el fail-fast de R6.2 — "Render .env" nombrando `SMTP_HOST`). Las seis claves `SMTP_*` verificadas en el `.env` de la VM. Nota: el apply del paso 1 destapó el incidente del boot volume (imagen sin pinear + metadata congelada) — post-mortem y fixes en los commits `ca00fdd`/`f2b0cdf` de `main`.
3. **Done 2026-09-03**: reset de contraseña pedido contra el owner de bootstrap (email real), fila `PASSWORD_RESET_REQUESTED | EMAIL | SENT | attempts=1 | last_error NULL` en `notification_logs`, y el correo recibido en la bandeja real de Gmail — R7.1 verificado de punta a punta. Hallazgo derivado: el link llegó con el default `http://localhost:3000` (el `.env` desplegado no definía `FRONTEND_BASE_URL`); arreglado en `ca00fdd` y verificado en el deploy 33731509810.
