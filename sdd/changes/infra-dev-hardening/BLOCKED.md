...
Orden recomendado:
1. **§3.3** — borrar en la consola OCI el budget/alerta creado a mano (evita duplicado).
2. **§5.4** — configurar el GitHub Environment `dev-apply` con required reviewers Jose + Marta (Settings del repo).
...
5. **§8.3** — lanzar el `workflow_dispatch` `apply` desde `main` (con aprobación del Environment): crea budget €1 + 2 alertas + vault + key, y aplica el security list. Confirmar `0 to destroy`.
