...
1. WHEN se audita `#28.18` (tenant isolation testeado), THE SYSTEM SHALL listar los 19 dominios de `backend/app/` y, para cada uno con estado propio scopado por tenant, confirmar que existe al menos un test que verifique que el tenant A no ve datos del tenant B — y añadir el test que falte donde el hueco sea real.
