...
Esta decisión razona sobre **colisión** de puertos, no sobre la **interfaz** en la que quedan publicados: que `make up` publique 8000/3000 en `0.0.0.0` de un runner con IP pública es un riesgo aparte, evaluado y aceptado en «Risks & mitigations» (con el porqué y las condiciones para reabrirlo).
...
- **Colisión de puertos entre corridas concurrentes en el mismo runner**: riesgo ya aceptado por `backend-tests.yml` (D2); no se introduce mitigación nueva aquí, por ser un problema preexistente del CI y no de este change. Lo que sí se añadió (review, ronda 1) es aislamiento de **proyecto** Compose —`COMPOSE_PROJECT_NAME: e2e-${{ github.run_id }}`—, porque la colisión de volúmenes con nombre entre dos PRs sí corrompe datos en caliente y no solo falla al arrancar; su contrapartida (volúmenes que ya no reclama nadie) la cierra el teardown con `--volumes` del propio job.
- **Exposición de red del stack de CI mientras el job corre** (añadido en review, ronda 2 — D2 razonaba solo sobre colisión de puertos): `make up` publica `backend` en `0.0.0.0:8000` y `frontend` en `0.0.0.0:3000` (interfaz deliberada de `docker-compose.yml`, que existe para abrir la UI desde un móvil de la LAN), así que durante el job hay un stack completo —bootstrapeado y sembrado, con cinco cuentas de login válidas aunque efímeras— escuchando en todas las interfaces de una VM con IP pública (`infra/environments/dev/RUNBOOK.md`). **Se acepta**, y el motivo es la postura de red de esa VM, no una propiedad del workflow: la security list de OCI **solo permite el 22 entrante** (el ingress de la app entra por un túnel Cloudflare *saliente*, ADR `0003-https-ingress-dev.md`), de modo que 8000 y 3000 no son alcanzables desde fuera de la VM. **No se reata a loopback** porque hoy no existe mecanismo para hacerlo sin infraestructura nueva: `PORT_OFFSET` desplaza el número de puerto y **conserva la interfaz** de cada servicio a propósito (`sdd/project.md`), y `docker-compose.worktree.yml` retira los mapeos por completo (`ports: !reset []`) — lo que dejaría sin camino a Playwright, que corre en el **host** del runner y no dentro de la red de compose —, además de que el `Makefile` solo lo carga en un worktree enlazado. Reabrir esta decisión si la security list llega a abrir otro puerto entrante o si el runner pasa a una red compartida: entonces el arreglo es un override de bind-address en `docker-compose.yml`, que hoy sería mecanismo nuevo para un riesgo ya cubierto por la capa de abajo.
