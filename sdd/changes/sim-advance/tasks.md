...
- Section 5 (+ round-1 fix, ficheros `docker-compose.deploy.yml` / `.github/workflows/deploy-dev.yml` / `infra/environments/dev/RUNBOOK-seed-demo.md`): `APP_ENVIRONMENT` es **obligatorio** (`:?`) en `backend`/`worker`/`beat` de `docker-compose.deploy.yml`, y el paso de render de `deploy-dev.yml` lo escribe en `dev-runtime.env` como literal `dev`. La guarda D5 en la VM de dev evalúa por tanto `"dev"` —valor permitido, no el default `"local"` de `Settings`— así que el comando NO queda bloqueado allí; el RUNBOOK §5 lo documenta con ese valor explícito, no como un default asumido.
