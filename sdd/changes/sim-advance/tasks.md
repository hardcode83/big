...
- Section 5 (+ round-1 fix + decisión post-review, ficheros `docker-compose.deploy.yml` / `.github/workflows/deploy-dev.yml` / `infra/environments/dev/RUNBOOK-seed-demo.md`): `APP_ENVIRONMENT` es **obligatorio** (`:?`) en los **cuatro** servicios que comparten la imagen de producción — `backend`/`worker`/`beat` y también `migrate` (BLOCKED decision resuelta en `a245adba`) — de `docker-compose.deploy.yml`, y el paso de render de `deploy-dev.yml` lo escribe en `dev-runtime.env` como literal `dev`. La guarda D5 en la VM de dev evalúa por tanto `"dev"` —valor permitido, no el default `"local"` de `Settings`— así que el comando NO queda bloqueado allí; el RUNBOOK §5 lo documenta con ese valor explícito, no como un default asumido, y ahora nombra los cuatro servicios.
...
- Round-2 fix (finding 1, design.md): la fila `Deploy` de "Changes by area" ya no dice "No se modifica" — describe el `${APP_ENVIRONMENT:?...}` obligatorio en `backend`/`worker`/`beat` y remite al Riesgo R5; conserva la cobertura de R2.3 por ausencia del target en el Makefile.
- Round-2 fix (finding 2, tasks.md): el bullet de Section 5 sobre `APP_ENVIRONMENT` reescrito a la realidad post-fix (obligatorio en compose + literal `dev` en `deploy-dev.yml`); el bullet redundante "Round-1 fix (finding 2…)" eliminado.
- Round-2 fix (finding 3, proposal.md): R3.2 ya no afirma `not_eligible` para una transición ya aplicada — el informe lo refleja como `candidates: 0` (verificado contra `test_a_second_run_writes_nothing`); R4.3 intacto, su `not_eligible: 1` es el caso distinto y correcto. `design.md` D4 no requirió cambio: describe el formato de línea genérico y no equipara ambos casos.
- Round-2 fix (finding 4, test_sim_advance_cli.py): nuevo test `test_a_second_cli_run_repeats_no_transition_and_writes_no_row` — dos pasadas de `cli._run` sobre el mismo tenant/reserva; la segunda reporta `candidates=0` / `not_eligible=0` y no escribe fila alguna (conteos antes/después, patrón de R4.4).
- Review fix (post-decision round): 3 new tests added — all-three-jobs-fail exit code 1, invalid (non-UUID) `--tenant` exit 1 via `_Parser.error`/`SystemExit`, and the live-clock label on the now-line when `--at` is omitted. README.md's `Arrancar` command list now includes `make sim-advance`.
