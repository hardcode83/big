...
- [ ] 8.1 Suite completa del backend en verde: `docker compose exec backend uv run pytest` (con el stack parado: `docker compose run --rm backend uv run pytest`).
- [ ] 8.2 Los fixtures de Channex siguen limpios y sus tests pasan sin modificación de comportamiento tras la extracción de la sección 1.
- [ ] 8.3 `grep` sobre los artefactos commiteados (`docs/beds24-request-cost.jsonl`, `fixtures/beds24/*.json`, el JSONL del sink) confirmando que no contienen el refresh token, el access token, el valor de la cabecera estática ni nada con forma de tarjeta.
- [ ] 8.4 Los scripts nuevos no viajan en la imagen: verificar que `backend/.dockerignore` los excluye, igual que a los de Channex.
