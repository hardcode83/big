...
2. La anotación del contrato de cleartext en `sdd/roadmap.md` → entrada `access-notifications`: `notification_logs.subject`/`body` solo admiten la forma enmascarada de un valor de la regla 3 de `security.md`. Es la mitad duradera del arreglo del hallazgo 1 del panel de seguridad — el docstring del modelo es la otra mitad.
...
- **Excepción única** (`notification_logs.subject`/`body`), y **solo para códigos de acceso**: renderizan un mensaje que el huésped debe recibir, así que se admite la forma enmascarada `****XX` de la regla 4. La regla 4 concede esa forma **únicamente** al código de acceso: al número de documento le exige ausencia de los listados, y a la contraseña WiFi no le concede nada. La primera redacción generalizaba a "cualquier valor de la regla 3", lo que habría autorizado un `document_number` enmascarado en una tabla con índice que hace barato listarla.
