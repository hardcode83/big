...
| Roadmap | `sdd/roadmap.md`, entrada `access-notifications` | Anotar que hereda el contrato de la **regla 11** de `steering/security.md` para `notification_logs.subject`/`body`/`last_error` — **al archivar** |
| Roadmap | `sdd/roadmap.md`, entrada `reservations-webhooks` | Anotar las **dos** obligaciones que hereda: la **regla 11** de `steering/security.md` para `webhook_events.payload`/`error`, y leer las filas con `tenant_id NULL` desde una sesión **nunca marcada** — **al archivar** |
| Roadmap | `sdd/roadmap.md`, entrada `user-management` | Anotar que hereda la **regla 11** de `steering/security.md` para `audit_logs.changes`. Es el primer escritor real de la tabla (cambios de rol) — **al archivar** |
...
2. La anotación en `sdd/roadmap.md` → entrada `access-notifications`: hereda la **regla 11** de `steering/security.md` para `notification_logs.subject`/`body`/`last_error`.
3. La anotación en `sdd/roadmap.md` → entrada `reservations-webhooks`, con las **dos** obligaciones que hereda: la **regla 11** para `webhook_events.payload`/`error`, y leer las filas con `tenant_id NULL` desde una sesión **nunca marcada** — nunca quitándole la marca a una que la tenga, porque `session.info` es por sesión y desactivaría el filtro para las 23 tablas.
4. La anotación en `sdd/roadmap.md` → entrada `user-management`: hereda la **regla 11** para `audit_logs.changes`. Es el primer escritor real de la tabla.


**Los sumideros de texto en claro** los levantó el panel de seguridad en tres rondas, y el contrato acabó viviendo en **la regla 11 de `steering/security.md`** — un único sitio, citado desde los tres docstrings y desde las tres filas de roadmap de arriba. Seis columnas, forma estructurada por defecto, una sola excepción.

Consolidarlo ahí **fue la salida al problema, no un adorno**: las dos primeras redacciones fallaron —una autorizaba un `document_number` enmascarado, la otra dejaba el criterio conceder por su cuenta y respondía *sí* a la contraseña WiFi— y las dos veces la corrección aterrizó en la copia explicativa mientras la versión superada sobrevivía en la que llega al siguiente implementador. Con el texto repetido en cuatro sitios eso era cuestión de tiempo; con uno solo, deja de poder pasar.

Lo que queda aquí es solo lo de este change: las tres tablas nacen sin escritor, así que la restricción se hereda con el change que primero escriba en cada una.
