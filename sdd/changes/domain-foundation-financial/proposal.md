...
- **Cifrado Fernet** de cualquier campo. Ninguna de las 10 declara una columna cifrada, pero **dos pueden acabar transportando un valor de la regla 3** de `security.md` si quien escriba en ellas se descuida: `notification_logs.subject`/`body` (el código de acceso viaja al huésped por una de esas notificaciones, PRD §17) y `audit_logs.changes` (un diff de `Guest.document_number` tomado donde el valor ya está descifrado). Lo levantó el panel de seguridad de la sección 1. **Este change no cifra ni enmascara nada** —no hay todavía ningún escritor—, pero fija el contrato en el docstring de ambos modelos para que `access-notifications` y quien registre auditoría lo hereden con su propio test. No es un supuesto: es una restricción escrita en el código que el revisor va a comprobar.
