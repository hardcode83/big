...
- **Cifrado Fernet** de cualquier campo. Ninguna de las 10 declara una columna cifrada, pero **seis pueden acabar transportando un valor de la regla 3** de `security.md` si quien escriba en ellas se descuida: `notification_logs.subject`/`body`/`last_error` (el código de acceso viaja al huésped por una de esas notificaciones, PRD §17), `audit_logs.changes` (un diff de `Guest.document_number` tomado donde el valor ya está descifrado) y `webhook_events.payload`/`error` (el PMS envía el cuerpo y §7.26 lo persiste literal). Lo levantaron los paneles de seguridad. **Este change no cifra ni enmascara nada** —no hay todavía ningún escritor—, pero fija el contrato en el docstring de los tres modelos, en dos formas: la **estructurada** es la regla por defecto y no conserva el valor en absoluto; la **única excepción** es `subject`/`body`, y solo para códigos de acceso, que admiten el `****XX` de la regla 4 porque renderizan un mensaje que el huésped debe recibir. No es un supuesto: es una restricción escrita en el código que el revisor comprueba.
