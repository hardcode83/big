...
Sigue `sdd/steering/security.md` regla 8 (excepción dev/test, análoga a la que `infra-dev-terraform` ya ejerce sobre la clave de la App): la clave `.pem` de la GitHub App que recibe este módulo acaba en el `tfstate` de `infra/github/` — vive en el mismo bucket `autohostai-tfstate-dev` (privado + versionado + IAM mínima de `svc-terraform-dev`) bajo la `key = "github.tfstate"`, separado del state dev. Esto **no** introduce una relajación nueva para esa clave concreta: ya está en el `tfstate` del dev module, y mantenerla en el mismo bucket (con `key` separada) es coherente con el patrón del módulo dev. Ámbito dev/test; antes de reutilizar el patrón en `staging`/`prod`, se revisará el gestor de secretos dedicado (compromiso heredado de `app-deploy-dev` y re-declarado en `infra-dev-terraform`).

**Enmendado en `/sdd:review` (2026-09-13, panel de seguridad):** el párrafo anterior describe la clave de la App, pero **sí** hay una relajación nueva y sin nombrar en la redacción original: R3.1 exige declarar los once secretos ya existentes del repo como `github_actions_secret`, incluido `OCI_PRIVATE_KEY` — la clave API de `svc-terraform-dev` que autentica el provider `oci` **y** el backend de state en **todos** los módulos de Terraform del proyecto. A diferencia del módulo `dev` (donde esa misma clave solo es config de provider/backend, nunca un atributo de recurso persistido), aquí `github_actions_secret.oci_private_key.plaintext_value = trimspace(file(var.oci_private_key_path))` **sí** la escribe en claro en `github.tfstate` como atributo gestionado (el `trimspace()` es el fix del bug de integridad relacionado — ver el final de este párrafo y Risk 7 — no cambia que el valor siga persistiendo en el state). Es una credencial de radio de daño mayor que la de la App (abre todo el tenancy OCI, incluido el propio bucket de state), y no estaba entre las cinco nombradas por la regla 8 hasta esta revisión. Aceptado explícitamente por el usuario en el gate de `/sdd:review`, con la misma condición que el resto de la excepción (bucket privado + versionado + IAM mínima); nombrada ahora como sexta pieza de la excepción en `sdd/steering/security.md` regla 8. Ver también Risk 6 más abajo, corregido para nombrarla.
...
### D5 — Branch protection: NO declararla en Terraform — el rechazo del plan Free es total, no parcial

**Enmendado en `/sdd:run` sección 4 (2026-09-13).** La redacción original de esta decisión asumía que la API rechazaría solo el sub-bloque de required reviewers, dejando el resto (required status checks, required linear history, enforce_admins) disponible en el tier Free. Verificado en vivo durante la sección 4 (`gh api repos/autohostai-labs/AutoHostAI/branches/main/protection` → `403 "Upgrade to GitHub Pro or make this repository public"`): la API rechaza el recurso **entero**, no un sub-bloque, mientras el repo siga privado en el plan Free — el mismo hecho que `infra/environments/dev/RUNBOOK.md` §0 ya documentaba desde antes de este change, sin que la redacción original de D5 lo hubiera cruzado contra el módulo GitHub.

**Chosen (enmendado):** NO declarar `github_branch_protection` (ni el recurso equivalente) en Terraform. Documentar en `infra/github/RUNBOOK.md` (R6, sección 5) que las tres reglas (required status checks, required linear history, enforce_admins) son convención no forzada mientras dure la restricción del plan/visibilidad — mismo trato que ya lleva "required reviewers" en el RUNBOOK del módulo dev. Un `terraform apply` que intentara crear el recurso fallaría por completo en ese recurso (no el "rechazo sin fallar, con nota en el plan" que la redacción original de R5.2 pedía), lo que además pondría en riesgo el resto del primer `apply` real de la sección 8 ("deploy from zero") si no se usa `-target` para aislarlo — complejidad no justificada por un recurso que la API ya deniega en el 100% de los casos, no en un subconjunto.

Rejected:
- **Declarar el recurso igualmente y capturar el error de `apply` como "constancia del rechazo"** — R5.2 original pedía que el rechazo apareciera "en el `plan`, sin fallar"; un `403` en la creación de un recurso completo hace fallar el `apply` de verdad (no es una nota inocua), lo que contradice la letra de R5.2. Mantener el recurso obligaría a `-target` para aislarlo del resto del primer apply, una complejidad nueva sin beneficio: la API nunca va a aceptar el recurso mientras el repo sea privado en Free.
- **Upgrade a GitHub Pro/Team** — desbloquea la API de verdad, pero es una decisión de coste/negocio ajena a este change (cambia la premisa Free-tier de `ADR 0002`); ofrecida al humano en el punto de decisión y no elegida.
- **Hacer el repo público** — también desbloquea branch protection en el plan Free, pero cambia la premisa de privacidad de `ADR 0002`; ofrecida al humano y no elegida.
- *(Redacción original, ya no vigente)* **Declarar lo que el plan Free soporte, documentar el resto** — la premisa de que había un "lo que soporta" no vacío para este recurso resultó falsa; se mantiene aquí tachada porque es lo que llevó al DESIGN-CONFLICT que esta enmienda resuelve, no porque siga siendo la decisión.
...
| Area | Files | Change |
|---|---|---|
| New module | `infra/github/main.tf` | Recursos `github_repository_settings`, `github_actions_secret`, `github_actions_variable`, y el recurso de packages (D9) que el provider vigente ofrezca para el acceso a GHCR del repo. **NO** `github_branch_protection` (D5 enmendado — la API la rechaza por completo en Free; convención documentada en el RUNBOOK, no recurso Terraform). `lifecycle { ignore_changes = [...] }` defensivo sobre atributos derivados del provider (D10). Provider `integrations/github` autenticado por App. |
...
3. **Branch protection: la API la rechaza por completo en Free, no solo required reviewers.** Verificado en vivo en la sección 4 (`403` en el endpoint completo). Mitigación: no se declara el recurso (D5 enmendado); el RUNBOOK documenta que las tres reglas son convención no forzada. Aceptado por `ADR 0002`.
...
6. **El secreto de la GitHub App queda en el `tfstate` de `infra/github/`** (igual que en `infra-dev-terraform` con la misma clave — `steering/security.md` regla 8, ámbito dev/test). Mitigación: vive en el bucket `autohostai-tfstate-dev` (compartido, privado + versionado + IAM mínima de `svc-terraform-dev`), bajo la `key = "github.tfstate"` separada del state dev. Mismo patrón que el bucket dev para esta clave concreta.

7. **`OCI_PRIVATE_KEY` (la clave del provider `oci` y del backend de state) también queda en el `tfstate` de `infra/github/`, y esto sí es una relajación nueva** (corregido en `/sdd:review` tras el panel de seguridad, 2026-09-13 — el riesgo 6 de arriba solo nombraba la clave de la App). `github_actions_secret.oci_private_key` la persiste como atributo de recurso; el módulo `dev` nunca lo hace con esta misma clave (ahí es solo config de provider/backend). Es la credencial de mayor radio de daño de las seis que cubre la excepción de la regla 8 — abre todo el tenancy OCI, no solo un servicio. Mitigación: misma que el riesgo 6 (bucket privado + versionado + IAM mínima), aceptada explícitamente por el usuario y nombrada como sexta pieza de la excepción en `steering/security.md` regla 8. Riesgo relacionado y ya corregido en el mismo panel: el workflow escribía la clave a fichero con `printf '%s\n'` (añade un salto de línea) y Terraform releía ese fichero para `plaintext_value`, así que cada `apply` reescribía el secret con un salto de línea adicional acumulado — un bug de integridad sobre la propia credencial que autentica `infra-dev.yml`, corregido recortando el salto de línea antes de escribir `plaintext_value` (ver `main.tf`).
