...
### D3 — Backend de state en su propio bucket `autohostai-tfstate-github`, paralelo al del dev

**Chosen:** `backend.tf` declara el backend nativo `oci` apuntando a un bucket separado, `autohostai-tfstate-github`, con la misma configuración parcial (`-backend-config`) que `infra/environments/dev/backend.tf`. El bucket se crea manualmente una sola vez (bootstrap irreducible — mismo motivo que el bucket del state dev: dependencia circular con su propio state), con **versioning activado**.
...
- **Reutilizar el bucket `autohostai-tfstate-dev`** — un solo state por bucket es el patrón Terraform; mezclar dos root modules en el mismo bucket requiere state key collision avoidance, complica la recuperación y rompe el `concurrency` entre `apply` simultáneos de los dos módulos.
...
Sigue `sdd/steering/security.md` regla 8 (excepción dev/test, análoga a la que `infra-dev-terraform` ya ejerce sobre la clave de la App): la clave `.pem` de la GitHub App que recibe este módulo acaba en el `tfstate` de `infra/github/` — el bucket `autohostai-tfstate-github` hereda el patrón del dev (privado + versionado + IAM mínima de `svc-terraform-dev`). Esto **no** introduce una relajación nueva: la clave ya está en el `tfstate` del dev module, y mantenerla en un único estado es lo que evita una segunda copia fuera de control. Ámbito dev/test; antes de reutilizar el patrón en `staging`/`prod`, se revisará el gestor de secretos dedicado (compromiso heredado de `app-deploy-dev` y re-declarado en `infra-dev-terraform`).
