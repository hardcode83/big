...
## Section 5 tasks 5.4/5.5 depend on section 6's apply job, which does not exist yet

- **phase**: run
- **type**: deferred
- **tasks**: 5.4, 5.5
- **what & why**: Task 5.4 requires triggering 'workflow_dispatch action: apply' on infra-github.yml and verifying the post-apply plan is empty; task 5.5 verifies no secret leaks in the resulting outputs. Checked the live workflow file: .github/workflows/infra-github.yml only declares 'check' (pull_request) and 'plan' (workflow_dispatch) jobs today — there is no 'apply' job. That job is section 6's deliverable (task 6.1: 'workflow_dispatch con action: plan|apply'). Implementing 5.1 (RUNBOOK.md), 5.2 (dev RUNBOOK cross-reference) and 5.3 (import.sh pointer) now since they have no such dependency; leaving 5.4/5.5 unchecked and moving on to section 6. Once section 6 delivers the apply job, 5.4/5.5 converge with section 8's tasks 8.1/8.2 (which perform the same real apply + empty-plan verification as part of 'deploy from zero') — they may end up satisfied by the same live run rather than a separate one.
- **exact resume command**: /sdd:run infra-github-iac 5.4
