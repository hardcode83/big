...
Un nuevo root module de Terraform `infra/github/`, ortogonal a los módulos por entorno en `infra/environments/<env>/` (siguiendo la convención de layout de `steering/infra.md`), que usa el provider `integrations/github` para declarar la superficie GitHub-side como código: settings del repo (descripción, topics, rama por defecto, visibilidad), secrets/variables de Actions (reflejando la tabla de `infra/environments/dev/README.md` §"Secrets de GitHub Actions esperados" y las variables de `app-deploy-dev`) y acceso a packages (GHCR). Branch protection **no** se modela como recurso Terraform (R5 enmendado): la API de GitHub rechaza `github_branch_protection` por completo mientras el repo siga privado en el plan Free de `ADR 0002` (verificado en vivo el 2026-09-13), así que las tres reglas quedan documentadas como convención no forzada en `infra/github/RUNBOOK.md`, no modeladas ni siquiera best-effort. La instalación de la GitHub App sobre el repo queda fuera de este módulo: el provider `integrations/github` v5.45.0 documenta que `github_app_installation_repositories` es incompatible con la autenticación por App (`app_auth`, D2) — está documentada en `infra/github/RUNBOOK.md` §1 como bootstrap irreducible (D11). La autenticación del provider usa la **misma GitHub App** que ya existe en la org y que `infra/environments/dev/cloud-init.yaml.tftpl` usa para mintear el installation token del runner — la App tiene los permisos que el provider requiere, y no se introduce un mecanismo de auth nuevo. El módulo se valida en CI con un workflow `pull_request` sobre `infra/github/**` (mismo patrón que `.github/workflows/infra-dev.yml`). El bootstrap irreducible (org, GitHub App, clave privada) se condensa en una sección única del nuevo `infra/github/RUNBOOK.md`, y `infra/environments/dev/RUNBOOK.md` §6 pasa a referenciarlo.
...
### R4 — Documentar la instalación de la GitHub App y gestionar el acceso a packages (GHCR) como código
...
1. THE SYSTEM SHALL documentar el estado de la instalación de la GitHub App sobre el repo en `infra/github/RUNBOOK.md` §1 como bootstrap irreducible (la API del provider `integrations/github` v5.45.0 documenta que `github_app_installation_repositories` no es compatible con la autenticación GitHub App Installation `app_auth`, que es la que D2 eligió). El job `verify-ghcr` de la sección 6 detecta en runtime si la App pierde los permisos que el CD necesita; el `terraform plan` post-apply NO modifica la instalación de la App.
...
- **Branch protection** — límite del plan Free documentado en `ADR 0002` y confirmado en vivo el 2026-09-13 (`403` de la API sobre el recurso completo, no solo el sub-bloque de required reviewers que la redacción original anticipaba). R5 (enmendado) no automatiza ninguna parte: las tres reglas (required status checks, required linear history, sin bypass por administradores) siguen siendo convención no forzada, documentada en `infra/github/RUNBOOK.md`.
