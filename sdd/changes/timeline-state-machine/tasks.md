...
- [x] 1.1 TDD: first add failing assertions for all 16
  ...
  [R1, R2, R3]
- [x] 1.2 TDD: first add failing construction/frozen-value tests for
  ...
  [R2, R5, R6, R7]
- [x] 1.3 TDD: first add failing validation tests for aware
  ...
  `docker compose exec backend uv run pytest tests/properties/test_state_machine.py tests/properties/test_transition_result.py -q`
  [R2, R5, R6, R7]
- [x] 1.4 TDD: first add failing tests for the structured exception families
...
- [x] 2.1 TDD: first add failing tests for immutable `TimelineEventData` and
  ...
  [R6, R7]
- [x] 2.2 TDD: first add failing tests for
  ...
  `docker compose exec backend uv run pytest tests/timeline/test_event_factory.py -q`
  [R6, R7]
- [x] 2.3 TDD: first add failing tests for timezone-aware timestamps, required
  ...
  `docker compose exec backend uv run pytest tests/timeline/test_event_factory.py -q`
  [R6, R7]
- [x] 2.4 TDD: first add a failing test that creates one existing non-property
...
- [x] 3.1 TDD: first add failing precedence tests for remaining active
  ...
  [R3, R4, R7]
- [x] 3.2 TDD: first add failing precedence tests for cleaning
  ...
  `docker compose exec backend uv run pytest tests/properties/test_state_resolution.py -q`
  [R3, R4, R7]
- [x] 3.3 TDD: first add failing reservation-state and boundary tests for the
  ...
  `docker compose exec backend uv run pytest tests/properties/test_state_resolution.py -q`
  [R2, R3, R4, R7]
- [x] 3.4 TDD: first add failing tests for property timezone conversion,
  ...
  `docker compose exec backend uv run pytest tests/properties/test_state_resolution.py -q`
  [R2, R3, R4, R7]
- [x] 3.5 TDD: first add failing tests for
  ...
  `docker compose exec backend uv run pytest tests/properties/test_state_resolution.py -q`
  [R2, R3, R4, R5, R7]
- [x] 3.6 TDD: first add failing tests for incomplete temporal data,
...
- [x] 4.1 TDD: first add parameterized failing tests for every approved
  ...
  `docker compose exec backend uv run pytest tests/properties/test_state_machine.py -q`
  [R1, R2, R3, R7]
- [x] 4.2 TDD: first add parameterized failing tests for every approved
  ...
  `docker compose exec backend uv run pytest tests/properties/test_state_machine.py -q`
  [R1, R2, R3, R7]
- [x] 4.3 TDD: first add parameterized failing tests for every approved
  ...
  `docker compose exec backend uv run pytest tests/properties/test_state_machine.py -q`
  [R1, R2, R3, R4, R7]
- [x] 4.4 TDD: first add parameterized failing tests for owner block,
  ...
  `docker compose exec backend uv run pytest tests/properties/test_state_machine.py -q`
  [R1, R2, R3, R5, R7]
- [x] 4.5 TDD: first add failing trigger-precondition tests for source entity
  ...
  `docker compose exec backend uv run pytest tests/properties/test_state_machine.py -q`
  [R2, R3, R7]
- [x] 4.6 TDD: first add a generated failing test over the canonical
...
- [x] 5.1 TDD: first add a failing happy-path test for
  ...
  `docker compose exec backend uv run pytest tests/properties/test_state_machine.py -q`
  [R2, R3, R6, R7]
- [x] 5.2 TDD: first add failing evaluate tests for cleaning completion and
  ...
  `docker compose exec backend uv run pytest tests/properties/test_state_machine.py -q`
  [R2, R3, R4, R6, R7]
- [x] 5.3 TDD: first add failing evaluate tests for manual block,
  ...
  `docker compose exec backend uv run pytest tests/properties/test_state_machine.py -q`
  [R2, R3, R5, R7]
- [x] 5.4 TDD: first add failing assertions that every accepted and rejected
...
- [x] 6.1 TDD: first add failing result tests that an accepted transition
  ...
  [R1, R6, R7]
- [x] 6.2 TDD: first add failing tests that optional `correlation_id` is copied
  ...
  `docker compose exec backend uv run pytest tests/properties/test_transition_result.py -q`
  [R6, R7]
- [x] 6.3 TDD: first add failing determinism tests proving identical complete
  ...
  [R2, R4, R6, R7]
- [x] 6.4 TDD: first add failing tests for invalid transition, invalid context,
...
- [x] 7.1 Run the complete focused domain suite and confirm all approved valid
...
- [x] 7.2 Run the full backend regression suite using the exact project command
  and keep every existing unit/integration test green — files verified:
  `backend/tests/` — command:
  `docker compose exec backend uv run pytest`
  [R1, R2, R3, R4, R5, R6, R7]
- [x] 7.3 Validate the dependency boundary by confirming no forbidden imports
  ...
  [R7]
- [x] 7.4 Confirm no speculative port/protocol or persistence abstraction was
  added — files:
  `backend/app/properties/domain/state_machine.py`,
  `backend/app/properties/domain/state_resolution.py`,
  `backend/app/properties/domain/value_objects.py`,
  ...
  [R7]
- [x] 7.5 Confirm existing entities, persisted enums, ORM, and migrations remain
...
- [x] 7.6 Review the original implementation diff, before section 8 remediation,
  ...
  [R7]
- [x] 7.7 Run whitespace/conflict validation and manually map the passing tests
