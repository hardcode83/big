...
- [x] 4.3 Paso `docker login ghcr.io -u x-access-token` con un **installation-token de la GitHub App** minteado en la VM (clave del Vault + helper) y `docker logout` (`if: always()`) al finalizar. — **Files:** `.github/workflows/deploy-dev.yml` — [R3]
