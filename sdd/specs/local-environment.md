...
- THE SYSTEM SHALL hacer que **los once targets que invocan `docker compose` desde el `Makefile`** (`up`, `down`, `logs`, `ps`, `sh`, `bootstrap`, `seed-demo`, `demo-reset`, `openapi`, `check-frontend-build`, `db-clean-test`) pasen por una única definición del comando, para que ninguno opere sobre un conjunto de ficheros distinto del que levantó el stack (ver §«Stacks en paralelo por worktree»). `check-rule11-ownership` **no** entra en la cuenta: los **cinco** targets que delegan en un script host-side —`check-version-parity`, `compose-stacks`, `check-compose-ports`, `ports` y `check-rule11-ownership`— no invocan `docker compose` desde el `Makefile` y por tanto no entran en la cuenta. La cuenta mide targets que lo invocan **desde el `Makefile`** y garantiza que todos pasen por una única definición; `check-compose-ports` lo invoca su script, desde Python, con entorno propio y deliberadamente desnudo, que es lo contrario de lo que la cuenta garantiza — meterlo dentro la haría medir dos cosas distintas.


  **Eran «diez» y son once**, y consta cómo se vio, porque el mecanismo importa más que la
  cifra: el que faltaba es `check-frontend-build`, que invoca `$(COMPOSE) exec -T` en su receta y
  no estaba ni en esta enumeración ni en la de los host-side. Salió al recontar target por target
  contra el `Makefile` al añadir `check-rule11-ownership` — no lo levantó ninguna prueba, porque
  no hay ninguna que lo pueda levantar. Es exactamente el fallo que esta spec describe y que la
  regla 11 lleva documentado: un recuento en prosa al lado de su fuente envejece sin que nada se
  ponga rojo, y por eso la obligación es recontar y nunca incrementar.
...
- `make sim-advance TENANT=<uuid> [AT=<instante ISO>]` ejecuta `python -m app.cli.sim_advance --tenant <uuid> [--at <instante>]` dentro del contenedor `backend` para avanzar a mano los tres jobs de reloj (`CHECKIN_WINDOW_OPENED`, `CHECKIN_TIME_REACHED`, `CHECKOUT_TIME_REACHED`) de un tenant sin esperar a `beat`, contra el backend ya levantado — igual que `bootstrap`/`seed-demo`/`demo-reset`, **no** forma parte de `make up`. El comando se niega a correr si `Settings.environment` no es `local` ni `dev`, guardia que cierra por construcción la posibilidad de avanzar el reloj de un tenant real (ver spec `celery-jobs`, sección «Avance manual del reloj (`sim-advance`)», y `docs/celery-jobs.md`).
...
- El worker ejecuta las **nueve** tareas periódicas que `celery-jobs` registra en `backend/app/worker.py` —las **cinco** nombradas por PRD §8.3, incluida `generate_price_recommendations`, que es la única del calendario que corre a una hora del día y no a una cadencia, más las cuatro que el PRD no nombra: `dispatch_notifications`, `provision_access_records`, `process_webhook_events` y `classify_incidents`—, con broker/backend en `REDIS_URL`; `beat` es quien las dispara (ver `specs/celery-jobs.md`). El fichero de estado de `beat` aparece como `backend/celerybeat-schedule` en el árbol de trabajo —el bind mount de `./backend` lo hace persistente en el host, no efímero— y está en `.gitignore`.
