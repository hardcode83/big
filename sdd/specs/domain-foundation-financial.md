...
- Las otras cinco columnas siguen sin escritor: la restricción la hereda el change que primero escriba en cada una (`access-notifications` para las tres de `notification_logs`, `reservations-webhooks` para las dos de `webhook_events`), con su propio test.
