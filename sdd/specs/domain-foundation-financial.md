...
- **Esta capacidad introduce siete columnas** de texto o JSON libre por el que puede colarse un valor sensible sin que la columna lo anuncie: `notification_logs.subject`/`body`/`last_error`, `audit_logs.changes` y `webhook_events.payload`/`error`/`event_type`. Siete son **las suyas**, no las del esquema: el censo completo lo lleva la tabla de la regla 11 y hoy es bastante mayor. La última de las siete entró tarde, por un motivo que conviene conservar: `event_type` no *parece* texto libre —se llama «tipo» y uno espera un enum—, pero la rellenaba el cuerpo del webhook, así que el censo se hace por **quién escribe la columna**, no por lo que su nombre promete.
- El contrato que las gobierna es **la regla 11 de `steering/security.md`**, y vive solo ahí: forma estructurada por defecto —el valor no sobrevive, ni enmascarado— más las excepciones que la propia regla enumera. Cuántas son y a qué columna aplica cada una lo dice la regla, no esta spec: aquí llegó a constar «una única excepción» y hace tiempo que no era una.
- **Quién escribe cada una lo dice la tabla de la regla 11 en `steering/security.md`, y solo ella.** Esta spec no lo repite a propósito: la propiedad de estos sumideros llegó a estar reafirmada en diecinueve artefactos y cada revisión encontraba uno más desincronizado —tres llegaron a afirmar algo falso—, y el change `rule11-ownership-single-source` (2026-08-17) quitó esa duplicación de raíz y dejó un test que la impide reaparecer. Lo que sí es información local y se queda aquí: estas columnas son sumideros de texto en claro y su contrato es la regla 11.
