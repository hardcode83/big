...
- Los campos que el PRD marca como cifrados en reposo (`Property.wifi_password_encrypted`, `Guest.document_number_encrypted`) siguen siendo columnas de texto **en claro**, y eso es deliberado: nada las lee ni las escribe todavía. La primitiva Fernet ya existe desde `pms-provider-resolution` (`app/core/crypto.py`, con `ENCRYPTION_KEY` obligatoria al arrancar), así que lo que falta no es el cifrado sino el change que use esos dos campos y los migre; se dejaron fuera de su alcance a propósito por no tener escritor. Las credenciales de proveedor, que sí lo tienen, nacen cifradas (`security.md` #3).
