...
- Cada módulo tiene al menos `domain/` (entidades Python puras + enums) e `infrastructure/` (modelos SQLAlchemy). `application/` y `api/` los añade el change que primero persiste o expone cada entidad: los introdujo `auth-tenancy` en `auth/` para `User` y para leer el estado de `Tenant`, y `properties/` tiene hoy las cuatro capas — `application/` desde los jobs programados y `api/` desde `properties-crud`, que es el primer caso de uso que necesitó exponerla.
...
- `User` y `Tenant` tienen puertos de repositorio y casos de uso desde `auth-tenancy` (`UserRepository`, `TenantStatusReader`; ver spec `auth-tenancy`), y `Property` los tiene desde `properties-crud`, que le dio su vía de escritura y su capa `api/` (ver spec `properties-crud`). Las entidades restantes siguen siendo solo estructura de datos, y sus puertos se difieren al change que primero necesite persistirlas.
...
- `backend/app/core/config.py` resuelve `DATABASE_URL`: fijada por `docker-compose.yml` a `postgres:5432` dentro de la red de compose; si no viene fijada (ejecución en host), cae a un valor por defecto contra `localhost:5432` con las credenciales de `POSTGRES_*` de `.env` — necesario porque el hostname `postgres` no resuelve fuera de la red de Docker. Ese camino sirve donde el `5432` del host esté publicado, y desde `worktree-port-offset` (2026-08-19) eso ya no es exclusivo del **worktree principal**: cualquier stack levantado con `make up PORT_OFFSET=<n>` publica Postgres en `5432+n`, así que ahí el default contra `localhost:5432` deja de encajar y hay que nombrar el puerto desplazado (`make ports` lo dice; ver spec `local-environment`); y en la práctica la suite se ejecuta dentro del contenedor, porque `uv` no está instalado en el host.
