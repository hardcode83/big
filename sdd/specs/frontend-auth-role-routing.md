...
- WHEN an authenticated visitor opens `/dashboard`, any route under `/(workspace)/*`, `/cleaner` or `/tech`, THE SYSTEM SHALL render the segment's content only when `useAuth().user.role` belongs to the segment's allowed set: `[TENANT_OWNER, PROPERTY_MANAGER]` for the workspace shell, `[CLEANER]` for the cleaner shell, `[TECHNICIAN]` for the technician shell.
...
- `frontend/app/(workspace)/layout.tsx`, `frontend/app/(field)/cleaner/layout.tsx`, `frontend/app/(field)/tech/layout.tsx` — the three layouts that pass the segment-specific `allow` set to `AuthGuard`.
