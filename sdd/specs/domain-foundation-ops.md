...
- Este change aportó a cada módulo solo `domain/` (entidades Python puras + enums) e `infrastructure/` (modelos SQLAlchemy). El `application/` y el `api/` de cada uno los añade el change que primero persiste/expone la entidad: `cleaning` lo hizo para las cuatro suyas y `access-notifications` para `AccessRecord`. `maintenance` (`Incident`) ganó su `application/` con [`guest-portal-api.md`](guest-portal-api.md) —un puerto de un solo método (`add`) y el caso de uso de creación— pero **sigue sin `api/` a propósito**, porque la única ruta que crea incidencias es la del portal del huésped; es el caso simétrico de `dashboard`, que tiene `api/` y no `infrastructure/`. `messaging-ai` (`Conversation`, `Message`) sigue sin ninguno de los dos.
