...
- `frontend/app/` — RootLayout (server, providers + locale + metadata), route groups `(workspace)`/`(public)`/`(field)`/`(guest)`, 18 placeholder pages plus three functional surfaces — the two dashboard surfaces (`/dashboard`, `/properties/[id]` — see `dashboard-web-frontend.md`) and the guest portal (`/guest/[token]` — see `guest-portal.md`), five shell layouts, `global-error.tsx`, per-segment `error.tsx`.
