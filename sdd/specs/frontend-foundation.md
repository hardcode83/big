...
- `frontend/app/` — RootLayout (server, providers + locale + metadata), route groups `(workspace)`/`(public)`/`(field)`/`(guest)`, 11 placeholder pages plus eleven functional surfaces — the three dashboard surfaces (`/dashboard`, `/properties/[id]`, `/timeline` — see `dashboard-web-frontend.md`), the read-only properties index (`/properties` — see `properties-crud.md`), the two read-only reservations surfaces (`/reservations`, `/reservations/[id]` — see `reservations-web` archived in `changes/archive/`), the two read-only incidents surfaces (`/incidents`, `/incidents/[id]` — see `incidents-web` archived in `changes/archive/`), the cleaning list (`/cleaning` — see `cleaning-manager-view.md`), the pricing screen (`/pricing` — see `revenue-pricing.md`) and the guest portal (`/guest/[token]` — see `guest-portal.md`), five shell layouts, `global-error.tsx`, per-segment `error.tsx`.
