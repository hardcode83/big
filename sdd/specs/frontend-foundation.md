...
- `frontend/app/` — RootLayout (server, providers + locale + metadata), route groups `(workspace)`/`(public)`/`(field)`/`(guest)`, 12 placeholder pages plus ten functional surfaces — the three dashboard surfaces (`/dashboard`, `/properties/[id]`, `/timeline` — see `dashboard-web-frontend.md`), the read-only properties index (`/properties` — see `properties-crud.md`), the two read-only reservations surfaces (`/reservations`, `/reservations/[id]` — see `reservations-web` archived in `changes/archive/`), the two read-only incidents surfaces (`/incidents`, `/incidents/[id]` — see `incidents-web` archived in `changes/archive/`), the cleaning list (`/cleaning` — see `cleaning-manager-view.md`) and the guest portal (`/guest/[token]` — see `guest-portal.md`), five shell layouts, `global-error.tsx`, per-segment `error.tsx`.
...
- `frontend/components/` — `ui/` (shadcn primitives: Button, Sheet, Tooltip, Separator, Skeleton, Badge), `states/` (`StatePanel`, `LoadingState`, `ErrorState`, `EmptyState`, `ModulePlaceholder`), `property-state-badge.tsx` (the single PRD §9.1 operational-state colour map, see `dashboard-web-frontend.md`).
