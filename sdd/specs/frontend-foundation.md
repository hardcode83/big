...
- `frontend/app/` — RootLayout (server, providers + locale + metadata), route groups `(workspace)`/`(public)`/`(field)`/`(guest)`, 14 placeholder pages plus seven functional surfaces — the two dashboard surfaces (`/dashboard`, `/properties/[id]` — see `dashboard-web-frontend.md`), the two read-only reservations surfaces (`/reservations`, `/reservations/[id]` — see `reservations-web` archived in `changes/archive/`), the two read-only incidents surfaces (`/incidents`, `/incidents/[id]` — see `incidents-web` archived in `changes/archive/`) and the guest portal (`/guest/[token]` — see `guest-portal.md`), five shell layouts, `global-error.tsx`, per-segment `error.tsx`.
