...
- **Desde `infra-github-iac`**: la **declaración** de qué secrets y variables de GitHub Actions existen pasa al nuevo root module `infra/github/` (provider `integrations/github`) — ya no se ponen a mano con `gh secret set`/`gh variable set`. Esta spec y `infra/environments/dev/README.md` §"Secrets de GitHub Actions esperados" siguen siendo la **referencia cruzada** de la lista de secretos esperados que `infra/github/` importa y gestiona; no pierden esa lista ni sus requisitos propios.
