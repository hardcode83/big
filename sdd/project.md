...
**Hidratación bajo `PORT_OFFSET`: este párrafo se ha reescrito varias veces (2026-08-23 «no hidrata», 2026-08-29 «no se reproduce», 2026-08-24 «sí, pero sólo con `next dev` + origen cruzado») — si te encuentras el síntoma, no repitas esa investigación desde cero: la medición vigente es la de abajo, con su fecha.**

**Medido 2026-09-05, MCP de Playwright, `next dev` con `make up PORT_OFFSET=77` (frontend en `:3077`), sin caer al build de producción: hidrata completa y limpiamente.** Login real por clic con `TENANT_OWNER` y `PROPERTY_MANAGER`, en las cuatro combinaciones rol × idioma: `document.querySelectorAll('*')` encuentra claves `__reactFiber$…` en 82 de los 116 elementos de `/login` ya antes de autenticarse, y un clic en el conmutador de idioma muta el DOM entero (título de pestaña, encabezados, card del dashboard, detalle de propiedad y su cronología) sin recarga manual, las cuatro veces. Único error de consola: `favicon.ico` 404 (ruido del servidor de desarrollo). **Nada de esto mide `allowedDevOrigins`**: sigue sin declararse en `frontend/next.config.ts` y sigue siendo una **hipótesis no verificada** — el síntoma que la motivó no se ha vuelto a reproducir desde 2026-08-29, así que no hay un antes/después observable que declararla pudiera demostrar, y hacerlo de todos modos exigiría además una variable de entorno nueva para la lista de orígenes (el desplazamiento de `PORT_OFFSET` varía por worktree) y un cambio en `specs/local-environment.md` por un arreglo cuyo efecto nadie puede medir. No se declara.

**Qué hacer, en orden: prueba `next dev` con `make up PORT_OFFSET=<n>` primero** —es lo que ha funcionado en todas las medidas recientes (`tech-app` 2026-08-29, `guest-portal-messaging` 2026-08-30, esta pasada 2026-09-05). Si la página no responde, cae al build de producción:

```bash
docker compose exec -T frontend npm run build          # imprescindible: el target `dev` del Dockerfile nunca lo corre por su cuenta
docker compose run --rm --no-deps -p <puerto>:3000 frontend sh -c 'npx next start -p 3000 -H 0.0.0.0'
```

y registra la condición exacta bajo la que falló, sin atribuirle causa. Las dos vías comparten el volumen `frontend_next`, así que se pisan: un `build` posterior invalida el `next dev` en marcha y viceversa — no son rutas independientes, se elige una y, si hace falta volver a la otra, se reinicia el contenedor.

> **Si te topas con el síntoma, lo primero: las credenciales que hayas tecleado acabaron en la URL.** `frontend/features/auth/components/login-form.tsx` es un `<form onSubmit={…} noValidate>` sin `method` ni `action`, y su `preventDefault()` vive dentro del manejador. Sin hidratar, el manejador no se engancha y el navegador hace su envío por defecto: un **GET a la propia URL con `email` y `password` como parámetros de consulta**. Quedan en la barra de direcciones, en el historial del navegador (en disco), en el log de peticiones de `next dev` y en cualquier traza de Playwright; y como `frontend` publica en todas las interfaces para poder abrirlo desde el móvil por la IP de la máquina, en ese uso viajan por la LAN en claro. Es entorno de desarrollo contra una base de datos de desarrollo, así que el techo es bajo — pero usa credenciales desechables y limpia el historial antes de seguir.

Dos cosas más que cuestan tiempo si no las sabes: la sesión vive en memoria (un `page.goto` a una ruta protegida cae a `/login`, el token no sobrevive una recarga dura; navega **por clic**, como un usuario) y el overlay de desarrollo de Next intercepta los clics de Playwright (`page.click` falla con «`<nextjs-portal>` intercepts pointer events»; `el.click()` en el DOM lo esquiva).
