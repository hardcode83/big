...
**Un aviso que estuvo aquí y era falso: «con `PORT_OFFSET` la página se sirve pero NO hidrata».** Se midió el 2026-08-23 en `cleaning-assign-preconditions` con `PORT_OFFSET=37` (submit nativo del formulario de login, conmutador de idioma muerto, ninguna prop de React en el `<form>`), se escribió aquí como norma, y desde entonces **aparcó comprobaciones visuales en más de un change** — `tech-app` dejó sus dos tareas de verificación sin hacer citando esta línea.
...
```bash
docker compose exec -T frontend npm run build          # imprescindible: ver abajo
docker compose run --rm --no-deps -p 3042:3000 frontend sh -c 'npx next start -p 3000 -H 0.0.0.0'
```

**La primera línea no estaba en el registro de `design-system-tokens` y sin ella la segunda no arranca.** El `target: dev` de `frontend/devops/Dockerfile` corre `npm run dev` y **nunca** ejecuta `npm run build` —el `RUN npm run build` sólo existe en la etapa `builder`, que este servicio no usa—, y el propio `docker-compose.yml` lo dice: «en dev … no hay nada horneado». Comprobado el 2026-08-29 montando el volumen `frontend_next` de este worktree en solo lectura: contiene únicamente `dev/`, sin `BUILD_ID`, sin `routes-manifest.json` y sin `prerender-manifest.json`, que es justo lo que `next start` exige. Tal cual estaba archivado, el comando falla con *«Could not find a production build in the '.next' directory»*. Allí funcionó porque alguien había construido antes y no lo escribió.

**Y las dos vías se pisan**, porque `/app/.next` es el volumen compartido `frontend_next`: el `build` escribe encima de lo que está sirviendo el `next dev` vivo y Next limpia lo obsoleto del `distDir`, así que después del build hay que reiniciar el contenedor de desarrollo, y un `next dev` posterior invalida el build de producción. No son rutas independientes: se elige una.
