...
- **`title` llega ya traducido, `description` no.** El backend compone el título al leer, contra un catálogo de 46 tipos × 2 idiomas en `backend/app/timeline/domain/rendering.py:70-265`, en el idioma que declara la petición (cabecera `X-Locale`, que el frontend rellena con el idioma resuelto de i18next), con degradación al `preferred_language` del usuario si la petición no declara uno soportado, y a `es` si tampoco ese lo es (`sdd/specs/dashboard-api.md:151-158`). `description` es texto de operador y vuelve **verbatim** en el idioma en que se tecleó. **El frontend no traduce títulos**: eso ya está hecho, y volver a hacerlo sería una segunda fuente de verdad.
...
- **Sí hay que exportar `PropertyTimeline`** desde `features/dashboard/index.ts`, que hoy solo exporta `DashboardView` y `PropertyDetailView` (líneas 1-2). *(Superado en el gate de `/sdd:design`, OQ1: el barrel exporta `TimelineView` y `PropertyTimeline` sigue interno a la feature. Lo que se conserva de este análisis es la convención —la página importa del barrel, nunca de una ruta interna—, no el símbolo concreto.)* La convención está demostrada en `app/(workspace)/properties/[id]/page.tsx:3`: la página importa del barrel de la feature, nunca de una ruta interna.
