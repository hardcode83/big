...
lista y detalle, sólo lectura**, consumiendo `GET /api/v1/reservations` y `GET /api/v1/reservations/{id}`. La ruta existe pero hoy es un `RoutePlaceholder`; el contrato ya está congelado y el cliente tipado ya lo conoce, así que el trabajo es la pantalla, no la negociación de formas. **Fuera de alcance**: `POST`, `PATCH` y `DELETE` —que la API sí sirve—, edición y cancelación desde la web, y cualquier PMS real …
