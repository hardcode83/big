...
`ci-backend-tests-conditional-gate` (2026-08-03) demostró que mover la decisión de área del disparador al interior del workflow elimina el coste de ejecutar suites que no tocan el diff, y solo el gate del backend lo aplica. Hoy un PR de solo prosa paga ~9 min agregados y un PR backend+frontend paga ~19 min; `frontend-tests` (~6 min por PR) es el siguiente candidato por tamaño y porque el propio comentario del workflow ya lo señala como precedente a copiar cuando empezara a doler. **Tres palancas adicionales en el mismo change**: caché de Playwright keyed por lockfile (~30-60 s/PR), caché de capas Buildx en `multiarch-build-check` y `deploy-dev` (~2-4 min/PR cuando toca el filtro), y consolidación del `npm ci` redundante dentro de `frontend-tests.yml` (~30-60 s/PR). La regresión histórica del runtime de la suite backend (205 → 424 → 709 s) queda fuera por decisión del usuario — pertenece a `backend-suite-runtime`, no aquí. **Tampoco**: bajar coverage (no hay threshold en CI), introducir nightly, mover validaciones críticas a nightly, cachear `node_modules`, ni tocar `infra-dev`/`demo-reset`/`api-contract`/`frontend-api-contract` (no está en el plan original, registrada el 2026-09-02 al abrir `/sdd:new`) …
