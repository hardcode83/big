...
- **Un test no puede depender del orden ni del proceso que le toque.** La suite corre en paralelo en CI (`pytest -n 2`; el número lo declara `.github/workflows/backend-tests.yml`, que es la fuente de verdad si algún día cambia), con una base de datos desechable y una base lógica de Redis **por worker**. En la práctica: nada de valores aleatorios dentro de `@pytest.mark.parametrize` ni de iterar `set`/`frozenset` para generar casos —el id del test cambiaría entre workers y la recolección aborta antes de ejecutar nada—, y nada de dar por hecho que un test ve lo que dejó otro. En local la suite sigue siendo en serie por defecto; `-n auto` es opcional.
