...
- **O — Open/Closed**: el patrón adapter ya obligatorio para todo sistema externo (`architecture.md`: "Todo sistema externo detrás de adapter") es Open/Closed en la práctica — un `Beds24Adapter` nuevo no toca `domain/` ni `application/`, solo implementa el puerto `PMSAdapter`.
- **L — Liskov Substitution**: `MockPMSAdapter` y el adapter real deben ser 100% intercambiables — mismas excepciones, misma forma de retorno, mismas precondiciones. Si el mock oculta un caso que el real no soporta, el contrato del puerto está mal definido, no es un detalle del mock. **Caso real de esta regla**: PRD §16 mete mensajería, reservas y ARI en un solo `Protocol`, pero la mayoría de proveedores del mercado no tienen mensajería OTA — implementarla con `NotImplementedError` rompería la sustitución. De ahí la separación de `PMSMessagingPort` decidida en [ADR 0006](../../docs/adr/0006-pms-channel-manager-provider.md), decisión 3.
...
- **Dominio sin invariante real** (p. ej. `NotificationLog` — básicamente una tabla con lectura/escritura, sin reglas): una entidad puede ser un `dataclass` simple sin métodos, y el repositorio no necesita nada más que get/save. No fuerces value objects ni eventos de dominio donde no hay nada que encapsular.

  **Salvedad sobre `AuditLog`** (añadida en `user-management`, 2026-07-31): esta lista lo citaba como ejemplo canónico de dominio sin invariante, y lo era mientras `audit_logs` no tenía ningún escritor. Ya lo tiene, y con él una invariante real: el **contrato de sumidero de texto en claro de la regla 11 de `security.md`** para su columna `changes`. Ese contrato no se sostiene como convención repetida en cada caso de uso — el panel de seguridad de `user-management` demostró tres vías de fuga distintas antes de que quedara cerrado por construcción — así que `app/audit/domain/` tiene value object (`ChangeSet`), fábrica y lista de campos auditables. La regla general sigue en pie: **la ceremonia táctica se justifica por la invariante que protege, no por el dominio en que vive.** Un dominio de esta lista que resulte tener una regla que proteger deja de pertenecer a ella, y se documenta como se documentó esta.

Lo que **sí** se mantiene igual en todos los dominios, con o sin invariantes: la regla de dependencia, y la carpeta `domain/application/infrastructure/api/` **en cuanto el dominio tiene algo que poner en cada capa**. La estructura uniforme es barata y ayuda a que un agente no confunda las convenciones entre módulos; la ceremonia táctica (value objects/eventos) es lo que se dosifica según haga falta.

**Excepción: dominios que todavía son solo estructura de datos.** Un módulo cuyas entidades existen pero que aún no tiene ningún caso de uso nace con `domain/` + `infrastructure/` a secas; `application/` y `api/` llegan con el primer caso de uso real, y con ellos los puertos de repositorio. Crear los cuatro paquetes vacíos por adelantado no lo verifica ningún test e invita a colocar mal el primer caso de uso. Es la forma de las 26 entidades de `domain-foundation-core`/`-ops`/`-financial`; `auth` ganó `application/`/`api/` cuando `auth-tenancy` los necesitó, `reservations` cuando llegó su CRUD, e `integrations` cuando llegó la importación por CSV. Lo que **no** es excepción: la regla de dependencia y la pureza de `domain/` rigen desde la primera línea, y `tests/test_layering.py` las verifica por glob.
