"""`render_checkin_reminder_email` / `render_checkout_reminder_email`
(`guest-scheduled-comms` R1, R2, R5, design D6).
...
own check-in/check-out date/time, keyed only by language — the same discipline
...
def test_the_spanish_checkout_template_is_returned_by_default_language() -> None:
    subject, body = render_checkout_reminder_email(
        "es", PROPERTY_NAME, CHECK_OUT_DATE, CHECK_OUT_TIME
    )
...
def test_the_english_checkout_template_is_a_different_text() -> None:
    es_subject, es_body = render_checkout_reminder_email(
        "es", PROPERTY_NAME, CHECK_OUT_DATE, CHECK_OUT_TIME
    )
    en_subject, en_body = render_checkout_reminder_email(
        "en", PROPERTY_NAME, CHECK_OUT_DATE, CHECK_OUT_TIME
    )

    assert en_subject != es_subject
    assert en_body != es_body
    assert PROPERTY_NAME in en_body
    assert "2026-09-22" in en_body
    assert "11:00" in en_body


def test_an_unrecognised_language_falls_back_to_spanish_for_checkout() -> None:
    for unrecognised in ("fr", "", "ES", "en-US", "not-a-language"):
        assert render_checkout_reminder_email(
            unrecognised, PROPERTY_NAME, CHECK_OUT_DATE, CHECK_OUT_TIME
        ) == render_checkout_reminder_email(
            "es", PROPERTY_NAME, CHECK_OUT_DATE, CHECK_OUT_TIME
        )


def test_the_checkout_subject_fits_the_column() -> None:
    """`notification_logs.subject` is `String(500)`."""
    for language in ("es", "en"):
        subject, _ = render_checkout_reminder_email(
            language, PROPERTY_NAME, CHECK_OUT_DATE, CHECK_OUT_TIME
        )
        assert len(subject) <= 500


def test_different_property_names_and_dates_produce_different_checkout_bodies() -> None:
    """The only guest-visible variables are the property's name and the reservation's own
    checkout date/time (R2.2) — changing either must change the rendered body."""
    _, base_body = render_checkout_reminder_email(
        "es", PROPERTY_NAME, CHECK_OUT_DATE, CHECK_OUT_TIME
    )
    _, other_property_body = render_checkout_reminder_email(
        "es", "Otra Propiedad", CHECK_OUT_DATE, CHECK_OUT_TIME
    )
    _, other_date_body = render_checkout_reminder_email(
        "es", PROPERTY_NAME, date(2026, 12, 1), CHECK_OUT_TIME
    )
    _, other_time_body = render_checkout_reminder_email(
        "es", PROPERTY_NAME, CHECK_OUT_DATE, time(18, 30)
    )
...
def test_checkin_and_checkout_renderers_produce_different_subjects() -> None:
    """The two builders must not collide on template text even with identical inputs."""
    checkin_subject, _ = render_checkin_reminder_email(
        "es", PROPERTY_NAME, CHECK_IN_DATE, CHECK_IN_TIME
    )
    checkout_subject, _ = render_checkout_reminder_email(
        "es", PROPERTY_NAME, CHECK_IN_DATE, CHECK_IN_TIME
    )

    assert checkin_subject != checkout_subject
