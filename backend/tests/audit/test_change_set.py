...
def test_an_unknown_entity_type_cannot_have_a_change_set() -> None:
    """`CONVERSATION` is the example on purpose: a real table with no audit trail yet.

    The name has now moved seven times, and each move is the test working as designed. It
    ...
    `PRICE_RECOMMENDATION` (its D12), then `OWNER_STATEMENT` until `revenue-statements`
    registered it with its sibling `EXPENSE` (design D7 of this change), and `REVIEW` — the
    placeholder this change picked while `revenue-reviews` was still open — until that
    sibling change registered it with `REVIEW_RESPONSE_DRAFT` (its R1.7/R3.5) and landed
    first, so the two names collided in the merge of #147 and this one moved once more.

    `CONVERSATION` is `conversations`, a table `messaging-ai` created and
    `guest-portal-messaging` writes; neither registered it as an audit entity. Rule 9 does
    not enumerate it, so whoever audits it argues from the precedent `PROPERTY` set rather
    than from a citation. Either way they will trip on this line, which is the intended
