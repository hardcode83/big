...
survivor was still saying "three", and it was not off by one. That enumeration is the audit
control for rule 1 of `steering/security.md`, which is why it stopped being prose — and the
count was wrong twice more inside the change that removed it, which is why no number is
restated here. `tests/test_unscoped_reads.py` is where it lives.
