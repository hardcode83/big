...
@router.patch(
    ...
    description=(
        "Correct the category or the severity, and put a price on the job (R1.4). A triage "
        "that sets **both** `category` and `severity` on an incident in `OPEN` also "
        "classifies it, moving it to `CLASSIFIED` (R3.5); setting only one of the two leaves "
        "it in `OPEN`. An `estimated_cost` above the tenant's threshold opens the "
        "owner-approval gate and moves the incident to `AWAITING_OWNER_APPROVAL` (R2.1)."
    ),
)
