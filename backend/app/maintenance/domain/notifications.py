...
def staff_message_notification(
    *,
    ...
    """What the other side of an incident's message thread is told (`staff-messaging` R4).

    The literal mirror of `app.cleaning.domain.notifications.staff_message_notification` —
    same shape as `technician_assignment_notification`/`incident_critical_notification`, and
    the same discipline on `body` (rule 11 of `sdd/steering/security.md`, design D8): it
    carries only `message_id` and `incident_id` plus a constant text, **never `content`** —
    the message itself lives behind the thread's own authorisation, not in a notification row
    a much wider set of code paths can read.
    ...
    **No `sla_deadline_at`** (design D8): a staff message has no response deadline, so there
    ...
    return NotificationLog(
        ...
        created_at=now,
        updated_at=now,
        subject="New incident message",
        body=(
            f"You have a new message on an incident. "
