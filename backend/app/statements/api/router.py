...
@router.post(
    ...
    description=(
        ...
        "`period_end` counts it in `skipped` and does not regenerate. The response is this "
        "batch report — counters, not a statement body; fetch the statement itself via "
        "`GET /owner-statements` or `GET /{id}`. The "
