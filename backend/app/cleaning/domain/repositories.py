...
@dataclass(frozen=True)
class SignedPhotoLocation:
    """Where one photo's bytes live, and which tenant owns them. Nothing else.

    The return of `UnscopedCleaningPhotoLocationQuery` below, and deliberately **not** a
    `CleaningPhoto`: the anonymous serving route needs exactly two facts to do its job, and
    handing it the entity would hand it `uploaded_by`, `photo_type` and `created_at` — data
    about a tenant nobody has authenticated against — for a route that returns bytes.
    """
...
class UnscopedCleaningPhotoLocationQuery(Protocol):
    """`photo_id → (storage_key, tenant_id)` **with no tenant scoping at all** (design D7b).

    The one read in this module that does not take a `tenant_id`, and the name says so on
    purpose: everything else here is built so that an implementation without the tenant is
    ...
    **Why it is not a fifth method on `CleaningPhotoRepository`.** Two reasons, and the second
    is the one that matters. It would break that port's stated acceptance criterion (task 2.1:
    four methods, each with a caller, every one taking the tenant first). And it would put an
    ...
    **What keeps it safe is the ORDER of its caller, not the query.** This read grants nothing
    by itself: `ServeLocalCleaningPhotoUseCase` resolves the location, rebuilds the key, and
    only **then** verifies the signature. The signature covers the whole key, which starts with
    `tenants/{tenant_id}/` (D3), so a valid one proves the caller was given a URL minted for
    that photo of that tenant. And it cannot be used as an existence oracle either, because the
    ...
    **That precondition is enforced, not inferred.** `require_unmarked_session`
    ...
    async def locate_without_tenant_scoping(
        ...
        """The photo's key and owning tenant, or `None` when no such photo exists.

        `None` rather than an exception, matching `CleaningPhotoRepository.get`: the caller
        turns both "absent" and "signature refused" into the same answer, so there is nothing
        for an exception type to carry.
        """
        ...
