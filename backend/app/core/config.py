...
class Settings(BaseSettings):
    ...
    # `EMAIL`/`CONSOLE` read the six `smtp_*` fields above instead (change
    # `smtp-delivery-adapter`); the real WhatsApp keys are still just reserved by rule 8 of
    # `steering/security.md`, with no adapter reading them yet.
    #
