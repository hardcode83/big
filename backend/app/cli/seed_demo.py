...
async def apply_plan(
    ...
    *,
    ...
    # whole reason it works. `find_by_email_globally` is one of the three deliberately unscoped
    # queries — but "unscoped" is a property of the statement, not of the method: once
    ...
    today = at.astimezone(ZoneInfo(tenant.timezone)).date()
...
async def _seed_direct_reservation(
    *,
    ...
    """
    if await _has_reservation_with_channel_id(
...
async def _has_reservation_with_channel_id(
    reservations: SqlAlchemyReservationRepository,
    tenant_id: uuid.UUID,
    property_id: uuid.UUID,
    external_channel_id: str,
) -> bool:
    """Idempotency for the DIRECT stay, keyed on an id that does not move with the calendar.

    Not on property + dates, which is R4.6's letter: those dates are relative to the day of
    the run, so that key would create a fresh duplicate every day (design D9). And through
    ...
    while True:
        ...
        if any(item.external_channel_id == external_channel_id for item in page.items):
            return True
...
async def _seed_ota_reservations(
    ...
    **The seed checks the keys itself and does not hand the ingestor rows that already
    ...
    async def resolve(row: ReservationDTO) -> Property | None:
        ...
        # between the two routes", so a third door is the extension the module anticipates.
...
async def _seed_accounts(
    ...
    *,
    ...
    for account in plan.accounts:
        ...
        if known[account.email] is not None:
...
def main() -> int:
    try:
        ...
        print(
            "seed-demo: fill them in your .env — no defaults are shipped for user "
