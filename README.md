...
- Frontend (Next.js): http://localhost:3000 — Application Shell; `/` resuelve la **landing pública** o el redirect según el bit de presencia `autohostai.session.present`: cookie ausente → landing directo sin red; cookie presente → el Server Component pregunta al backend `GET /api/v1/auth/me` (timeout 2 s); 2xx → `307` a `/dashboard`; 401 → purga la cookie y renderiza la landing (defensa contra JWT revocado con cookie persistente); 5xx o timeout → `307` a `/dashboard` sin tocar la cookie (un fallo del backend no equivale a logout). Ver `sdd/specs/frontend-auth-role-routing.md` §«Server-side discrimination of a stale presence cookie at the application root». El **dashboard** (`/dashboard`) y el **detalle de propiedad** (`/properties/[id]`) son funcionales en modo solo lectura **salvo dos excepciones**: la sección de desajustes de la card del dashboard, que desde `blocked-transitions-web` ofrece al `PROPERTY_MANAGER` cancelar la limpieza o resolver la incidencia que están bloqueando la siguiente entrada — la propietaria ve el aviso con su `READ_PROPERTIES` pero no los botones (ver [`docs/dashboard.md`](docs/dashboard.md) y [`docs/properties.md`](docs/properties.md) §«Aviso de desajustes en la card del dashboard»)—, y las acciones de **editar** y **retirar** del detalle de propiedad, que desde `properties-create-web` ve y usa quien tiene `MANAGE_PROPERTIES` (ver [`docs/properties.md`](docs/properties.md) §«Dar de alta, editar y retirar desde la web»); el **listado de propiedades** (`/properties`) es el índice del portfolio — seis columnas, filtros por `status` y estado operacional, paginación, y el nombre enlazando al detalle — y desde `properties-create-web` aloja además el **alta** de una vivienda, gateada por `MANAGE_PROPERTIES` igual que la edición y la retirada del detalle; quien no tiene ese permiso ve ambas pantallas en modo solo lectura, y asignar credenciales de PMS sigue siendo sólo CLI (ver [`docs/properties.md`](docs/properties.md) §«Ver el portfolio desde `/properties`» y §«Dar de alta, editar y retirar desde la web»); la **pantalla de reservas** (`/reservations`, con su detalle `/reservations/[id]`) lista y abre reservas del tenant — filtros por `status` y rango de fechas `date_from`/`date_to`, paginación; el `PROPERTY_MANAGER` puede crear, editar y cancelar desde la web mediante `POST`/`PATCH`/`DELETE`, mientras `TENANT_OWNER` mantiene acceso de solo lectura (ver [`docs/reservations.md`](docs/reservations.md)); la **pantalla de incidencias** (`/incidents`, con su detalle `/incidents/[id]`) lista y abre las del tenant — filtros por `status` y `severity`, paginación; quien tiene `MANAGE_INCIDENTS` ve además en el detalle las **cuatro operaciones del manager** (`classify`, `triage` vía `PATCH`, `assign`/reasignar, `cancel`), gateadas por estado; las **seis de `EXECUTE_INCIDENTS`** (`accept`, `start`, `wait-parts`, `resume`, `resolve` y `reject`) y la respuesta de aprobación (`POST /owner-approvals/{id}/respond`) quedan fuera de alcance **desde esta pantalla** — las conduce la app del técnico, más abajo, y la respuesta de aprobación tiene ya su propia pantalla (ver [`docs/maintenance.md`](docs/maintenance.md)); la **cola de aprobaciones** (`/approvals`) es funcional contra el backend real — la cola pendiente y un historial de las últimas cinco respondidas, con el nombre de la propiedad y su código interno (nunca el UUID), la categoría/severidad/título de la incidencia traducidos y el importe; sólo la `TENANT_OWNER` ve los botones de aprobar/rechazar (`RESPOND_OWNER_APPROVALS`), el `PROPERTY_MANAGER` ve la cola en modo lectura, y rechazar cancela la incidencia sin vuelta atrás (ver [`docs/maintenance.md`](docs/maintenance.md) §«Cómo se ven desde la web»); la **app del técnico** (`/tech`, con su detalle `/tech/incidents/[id]`) es funcional contra el backend real — la lista de las incidencias asignadas con la vivienda de cada fila, el ciclo del técnico (`accept`, `reject`, `en-route`, `wait-parts`, `resume`), el cierre con coste y materiales (`resolve`), la galería y la subida de fotos antes/después, y la puerta de aprobación de la propietaria tal como la resuelve el backend (ver [`docs/maintenance.md`](docs/maintenance.md) §«La app del técnico»); el **portal del huésped** (`/guest/[token]`) es funcional contra las seis rutas anónimas del backend, sin cuenta ni JWT — consulta de la estancia, check-in legal, alta de incidencia y, desde `guest-portal-messaging`, la **conversación con el alojamiento**, que el huésped escribe él mismo y que se refresca por sondeo cada 15 s mientras la pestaña está visible (ver [`docs/guest-portal.md`](docs/guest-portal.md)); las **limpiezas** (`/cleaning`) son funcionales contra el backend real — lista paginada con filtros por vivienda y estado, y asignación/reasignación para el `PROPERTY_MANAGER` (ver [`docs/cleaning.md`](docs/cleaning.md) §«Operar las limpiezas desde `/cleaning`»); la **app de la limpiadora** (`/cleaner`, con su detalle `/cleaner/tasks/[id]`) es funcional para el rol `CLEANER` — la lista «mis tareas» con la vivienda y la ventana de cada fila, el detalle con piso + checklist + categorías de foto + galería, el ciclo del rol (aceptar / rechazar / iniciar / completar), el marcado de ítems del checklist, la subida de fotos por categoría con sus mensajes de error localizados, y el reporte de incidencia inline (ver [`docs/cleaner.md`](docs/cleaner.md)); el **historial de una vivienda** (`/timeline`) monta el mismo timeline que el detalle de propiedad sobre la vivienda que elijas en un selector — vocabulario cerrado de tipos de evento, filtros por actor y severidad, rango de fechas y paginación; el backend solo sirve el historial de una vivienda a la vez, así que no hay timeline global (ver [`docs/dashboard.md`](docs/dashboard.md)); los **precios recomendados** (`/pricing`) son funcionales contra el backend real — dos pestañas bajo la misma ruta: la **cola de recomendaciones**, con filtros por vivienda, rango de fechas y estado, paginación, los tres movimientos de decisión (aprobar, rechazar y marcar como publicada) para el `TENANT_OWNER` y el `PROPERTY_MANAGER`, y la regeneración bajo demanda; y las **reglas** que los producen, en modo solo lectura con filtros por vivienda y actividad — el interior de sus columnas JSONB se cuenta, nunca se pinta (ver [`docs/pricing.md`](docs/pricing.md)); las **reseñas** (`/reviews`) son funcionales contra el backend real — dos pestañas bajo la misma ruta: **Borradores**, la cola de decisión sobre las reseñas que la IA ya clasificó y redactó (filtros por vivienda, canal, sentimiento, rango de rating y rango de fechas; aprobar, ignorar y editar el borrador para el `TENANT_OWNER`, solo editar para el `PROPERTY_MANAGER`); y **Reseñas**, el listado completo con los mismos filtros más `status`, el detalle con el texto del huésped y el resumen de IA, y **Marcar como publicada** tras aprobar, con vista previa en diálogo antes de confirmar; el alta manual de una reseña (canal, valoración, texto e idioma opcionales) está reservada al `PROPERTY_MANAGER` (ver [`docs/reviews.md`](docs/reviews.md)); las **liquidaciones** (`/statements`) son funcionales contra el backend real — el listado filtrable por vivienda, período y estado (`DRAFT`/`READY`/`SENT`), paginado; el detalle con el resumen (los once importes contractuales) y los `breakdown` de reservas y gastos; las descargas CSV/PDF opacas (bytes + `Content-Disposition`, sin parseo en cliente) sobre `GET /api/v1/owner-statements/{id}/export.{csv,pdf}`. El acceso exige el permiso `READ_OWNER_STATEMENTS`, que el backend aplica en `/api/v1/owner-statements`, y el `AuthGuard` del `layout.tsx` del workspace lo reserva a los roles `TENANT_OWNER` y `PROPERTY_MANAGER` (ver [`docs/revenue-statements.md`](docs/revenue-statements.md) §«Pantalla `/statements`»); los **ajustes del tenant** (`/settings`) son funcionales contra el backend real — dos secciones apiladas bajo la misma ruta: la de **Usuarios**, el directorio del personal con filtros por rol y estado, paginación y el detalle de cada fila, más el alta con revelación única de la contraseña temporal, la edición con cambio de rol, la desactivación y el reset de contraseña asistido para la `TENANT_OWNER` (`MANAGE_USERS`); y la de **Tenant**, la configuración operativa que PRD §7.2 pone en manos de la propietaria —umbral de aprobación, SLAs, `auto_create_cleaning_task`, conmutadores de notificación e idioma por defecto— sobre `GET`/`PATCH /api/v1/tenants/{id}`. El `PROPERTY_MANAGER` ve las dos secciones en modo solo lectura, y `CLEANER`, `TECHNICIAN` y `SUPER_ADMIN` no alcanzan la ruta; la conexión de PMS (`/settings/integrations`) sigue siendo un placeholder a propósito, porque las credenciales de proveedor son CLI-only (ver [`docs/user-management.md`](docs/user-management.md) y [`docs/pms-credentials.md`](docs/pms-credentials.md)); el resto de rutas de módulos muestran un placeholder "en preparación". El *shell* renderiza sin backend; las superficies que consumen la API (propiedades, reservas, incidencias, cola de aprobaciones, app del técnico, portal del huésped, limpiezas, timeline, precios recomendados, reseñas, liquidaciones y ajustes del tenant) muestran su estado de error accesible si no está disponible.
...
`properties-web`: `/properties` lo sirve con filtros por `status` y estado operacional y
paginación. Desde `properties-create-web` la escritura también tiene pantalla: el **alta**
(`POST`) se hace desde el propio listado, y la **edición** y la **retirada** (`PATCH`, con
confirmación explícita para `status: INACTIVE`) desde el detalle — las tres gateadas por
`MANAGE_PROPERTIES`. Asignar credenciales de PMS sigue siendo sólo CLI.
...
- `backend/` — FastAPI + Celery (Python, `uv`). Dockerfile en `backend/devops/Dockerfile`. Código de dominio en `backend/app/<dominio>/` con las cuatro capas `domain/` → `application/` → `infrastructure/` → `api/` (regla de dependencia y fontanería en [`docs/adr/0004-backend-layering-pattern.md`](docs/adr/0004-backend-layering-pattern.md) y `sdd/steering/backend-architecture.md`). Son **18 dominios de negocio** —los directorios bajo `backend/app/` menos `cli`, `core`, `provenance` y `scheduler`, que son infraestructura y no dominio—, recontados contra el árbol y con su test de aislamiento cada uno en [`docs/dod-audit.md`](docs/dod-audit.md) §28.18. **Este párrafo decía 19 y su roster omitía `statements` y `audit`**: el número salía de una cuenta que ya no cuadraba con el árbol y el roster no se actualizó cuando `revenue-statements` dio a `statements` sus cuatro capas. Los que todavía son **solo estructura de datos** —entidades y esquema, sin ningún caso de uso que los use— nacen con `domain/` + `infrastructure/` a secas, y ganan `application/`/`api/` cuando llega el primer caso de uso real: hoy **`audit` es el único que sigue ahí** —se escribe desde `access`, `auth` y `platform` y no tiene lectura propia—, y los quince `auth`, `properties`, `reservations`, `integrations`, `tenants`, `cleaning`, `access`, `guests`, `notifications`, `timeline`, `maintenance`, `messaging`, `pricing`, `reviews` y `statements` son los que tienen las cuatro —`properties` ganó su `api/` con `properties-crud`; `access`, `guests` y `notifications` con `access-notifications`, que trajo la operación de accesos (PRD §15), el registro legal de huéspedes (§17) y la entrega de notificaciones (§14); y `timeline` con `dashboard-api`, que le añadió `application/` y `api/` de golpe: hasta entonces sus eventos los escribían los casos de uso de *otros* dominios y nadie los leía de vuelta (PRD §10). Ese mismo change añade el dominio **`dashboard`** —el decimoséptimo—, que es el lado de lectura del agregado de PRD §9 y el primero con `domain/`, `application/` y `api/` pero **sin `infrastructure/` propia**: no tiene tabla ni entidad, compone los puertos de los otros siete dominios. **`platform` es el segundo de esa forma**, y el último dominio en entrar: es la superficie de administración cross-tenant de PRD §23 —`POST /api/v1/platform/tenants` y `POST /api/v1/platform/tenants/{tenant_id}/users`— y no tiene tabla propia porque escribe en las de `tenants`, `auth` y `audit` a través de sus repositorios. Es la única superficie de la aplicación cuyo actor autentica con **sesión no marcada** (`tenant_id=None`), así que el filtro global de tenant no la cubre y el `tenant_id` de cada fila lo pone el caso de uso a partir del segmento de la ruta; el permiso `MANAGE_PLATFORM` lo tiene `SUPER_ADMIN` y nadie más. **`maintenance` fue el caso simétrico y ya no lo es**: desde `guest-portal-api` tenía `domain/`, `application/` e `infrastructure/` y **ningún `api/`**, porque su único caso de uso —crear la incidencia que abre el huésped— se exponía por el router del portal. El change `maintenance` le da esa cuarta capa junto con el resto de su flujo: clasificación (automática por job y manual), triaje, aprobación de la propietaria por encima del umbral, asignación con plazo de SLA y el ciclo del técnico hasta el cierre. Son **dos routers y no uno** —`/incidents` y `/owner-approvals`— porque son dos agregados: una incidencia puede levantar dos aprobaciones, la del presupuesto y la del coste real (ver [`docs/maintenance.md`](docs/maintenance.md)). **`messaging` es el caso siguiente**: desde `domain-foundation-ops` tenía `domain/` + `infrastructure/` y ningún escritor, y el change `messaging-ai` le da `application/` y `api/` con la atención de primer nivel al huésped de PRD §13 — detección de idioma, clasificación de intent, seis condiciones de escalación, respuesta desde un catálogo cerrado de plantillas y los siete endpoints de bandeja de PRD §16, bajo un solo router `/conversations` porque `Conversation` es el agregado y un mensaje no tiene identidad fuera de su hilo. Los mensajes entran por el panel o por la API: **no hay ingesta automática desde OTA**, y una conversación de `AIRBNB_MSG`/`BOOKING_MSG` queda muda a propósito hasta que llegue `beds24-messaging-adapter` (ver [`docs/messaging-ai.md`](docs/messaging-ai.md)). **`pricing` es el último en ganar las cuatro capas**: desde `domain-foundation-financial` tenía `domain/` + `infrastructure/` y ningún escritor, y el change `revenue-pricing` le da `application/` y `api/` con el Modo 1 del PRD §19 — la fórmula determinista de PRD §7.17 con sus guardrails, un horizonte de 60 días por vivienda y la aprobación humana de cada precio. **El sistema recomienda y no publica**: quien aprueba sube el precio a la OTA a mano y lo marca `APPLIED_EXTERNAL`. Son **dos routers** —`/pricing-rules` y `/price-recommendations`— porque son dos agregados: la regla que edita una persona y el horizonte que reescribe el job nocturno (ver [`docs/pricing.md`](docs/pricing.md)). El **scheduler** vive en `backend/app/scheduler/` — capa de entrega para el reloj, el equivalente de `api/` para Celery beat: dieciséis tareas —las cuatro de PRD §8.3 más `dispatch_notifications`, `provision_access_records`, `process_webhook_events`, `classify_incidents`, `classify_reviews`, `reconcile_owner_approvals_for_expenses` y `sync_pms_reservations` (cada 6h, ver línea de `make pms-sync` arriba), que el PRD no nombra y declaran como divergencia `access-notifications`, `reservations-webhooks`, `maintenance`, `revenue-reviews`, `revenue-statements` y `pms-sync-schedule` respectivamente, más `send_checkin_reminders` (`guest-scheduled-comms`, el propio nombre de PRD §8.3 para el job que nunca tuvo código), y `send_checkout_reminders`/`deliver_access_instructions` (también `guest-scheduled-comms`, declaradas como divergencia el mismo modo que las anteriores — la primera cubre `CHECKOUT_REMINDER`, que PRD §8.3 no nombra pese a que PRD §14 sí declara el tipo; la segunda entrega automáticamente las instrucciones de acceso de PRD §15), más `generate_price_recommendations`, que **sí** es de PRD §8.3 y es la única que corre a una hora del día (06:00 UTC) en vez de por cadencia, y `generate_owner_statements` (`revenue-statements`), la única que corre un día concreto del mes—, su calendario y el lock que evita solapes (ver [`docs/celery-jobs.md`](docs/celery-jobs.md)). Comandos operativos en `backend/app/cli/` y `backend/app/integrations/cli/`; adapters de sistemas externos en `backend/app/integrations/`, que además guarda las tablas `webhook_endpoints` y `webhook_events`; migraciones en `backend/alembic/`. Dentro de `integrations` vive también **`app/integrations/infrastructure/storage/`** — el almacenamiento de ficheros, con `LocalFileStorage` (escribe en el volumen `/app/media`) y `S3FileStorage` detrás del mismo puerto, y la factoría que elige uno u otro por el `storage_type` del tenant. Está en `integrations` y no en `cleaning` porque el puerto es compartido, y desde `incident-photos` (2026-08-23) esa razón ya está cobrada: sus llamantes son **dos** —las fotos de limpieza y las fotos de incidencia de `maintenance`—, y `revenue` (`expenses.receipt_storage_key`) sigue nombrado como el siguiente. Junto al almacenamiento vive también la **capa de servido firmado compartida** (`app/integrations/application/signed_serving.py` y `app/integrations/api/signed_media.py`): el cuerpo `403` constante, el `nosniff` y el `Cache-Control` derivado de la firma se escriben una vez y los dos dominios montan su ruta anónima con ella (ver [`docs/cleaning.md`](docs/cleaning.md) y [`docs/maintenance.md`](docs/maintenance.md)). El alta manual de reservas comparte el servicio `ResolveOrCreateGuest` y el adapter PostgreSQL `postgres_guest_email_exclusion.py` para resolver huéspedes por email normalizado y serializar creaciones concurrentes. **`backend/scripts/`** queda deliberadamente **fuera de `app/`**: son herramientas de un solo uso contra servicios externos (provisión y sondeo del sandbox de Channex — ver [`docs/channex-staging.md`](docs/channex-staging.md)) o de medición puntual (`measure_tenant_filter.py`) que no deben viajar en el paquete desplegado.- `frontend/` — Next.js App Router (TypeScript strict, Tailwind, shadcn/ui, TanStack Query, Zustand, react-i18next ES/EN). Application Shell organizado por capas `app/` → `features/` → `components/`·`lib/`. En `app/` vive además la **única pieza de servidor del frontend**: `app/api/[...path]/route.ts`, el proxy same-origin que reenvía `/api/` al backend por la red interna — es lo que hace que el navegador alcance la API sin exponer el backend, y su alcance está fijado por `app/proxy-scope.test.ts` ([`docs/ingress-https.md`](docs/ingress-https.md)). `lib/i18n/active-locale.ts` publica el locale activo de i18next de forma síncrona (`use-active-locale.ts` lo expone a componentes) para que `lib/api/authenticated-client.ts` lo mande en la cabecera `X-Locale` de toda petición autenticada — es lo que consume `resolve_locale()` en el backend (`frontend-verification-fixes`, ver `docs/dashboard.md`). Convenciones detalladas en [`frontend/README.md`](frontend/README.md). Dockerfile en `frontend/devops/Dockerfile`.
- `frontend/features/statements/` — módulo frontend de statements: listado filtrable y paginado por `property_id`, `period_start_from/to` y `status`; detalle con `reservations[]` y `expenses[]`; descargas opacas CSV/PDF.
...
- `backend/app/core/i18n.py` — mecanismo compartido de composición de texto en el idioma de la petición: `Locale` y `Catalog` (plantillas cerradas por idioma, sin mensajes propios) nacieron con `dashboard-api`; `frontend-verification-fixes` le añadió `resolve_locale(requested, stored)`, la cadena de degradación que expone `RequestLocaleDep` (cabecera `X-Locale` → `preferred_language` guardado → `es`). No aloja plantillas de ningún dominio — esas viven en `dashboard/domain/labels.py` y `timeline/domain/rendering.py` — ni entidades de negocio, per `steering/architecture.md`.
...
Los 20 ítems del PRD §28 están auditados uno a uno —con el fichero, el test y el comando que
sirven de evidencia a cada uno— en [`docs/dod-audit.md`](docs/dod-audit.md). El documento lleva
además las dos tablas de detalle que el PRD pide explícitamente: los **18 dominios de negocio**
con su test de aislamiento de tenant (§28.18) y las **transiciones de `PropertyStateMachine`**
—válidas e inválidas— con el test que cubre cada una (§28.19). Se audita, no se reconstruye: su
valor es que una lectura futura no tenga que volver a medirlo desde cero.
