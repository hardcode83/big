...
- Frontend (Next.js): http://localhost:3000 — Application Shell; `/` redirige a `/dashboard`. El **dashboard** (`/dashboard`) y el **detalle de propiedad** (`/properties/[id]`) son funcionales en modo solo lectura sobre datos mock (ver [`docs/dashboard.md`](docs/dashboard.md)); el **portal del huésped** (`/guest/[token]`) es funcional contra las cuatro rutas anónimas del backend, sin cuenta ni JWT (ver [`docs/guest-portal.md`](docs/guest-portal.md)); el resto de rutas de módulos muestran un placeholder "en preparación". No requiere backend para renderizar.
