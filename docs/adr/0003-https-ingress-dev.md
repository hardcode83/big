...
**Cloudflare Tunnel (`cloudflared`) como ingress HTTPS del entorno dev**, con el hostname `autohostai.digitalsec.net` sobre la zona `digitalsec.net` que el equipo ya tiene en Cloudflare.
...
El **Universal SSL gratuito cubre solo el apex y los subdominios de primer nivel**. Por eso el hostname es `autohostai.digitalsec.net` (una sola etiqueta bajo el apex), y una `validation` de Terraform lo hace cumplir en el `plan`. Consecuencia para cuando exista un segundo entorno: **aplanar** (`autohostai-staging.digitalsec.net`) o usar otra zona son 0 €, mientras que **profundizar** (`dev.autohostai.digitalsec.net`) exigiría Total TLS o Advanced Certificate Manager, **ambos de pago**. La estrategia multi-entorno queda deliberadamente aplazada, pero con ese dato sobre la mesa.
...
- **Los ajustes de HTTPS son de zona, no de hostname.** Cloudflare no los ofrece por hostname en el plan Free, así que forzar HTTPS y TLS mínimo 1.2 **aplica a todo `digitalsec.net`**, incluidos otros servicios del dominio. Aceptado a sabiendas: es la postura deseable para todos ellos. Si alguno necesitara HTTP en claro, este ajuste es lo primero a revisar.
...
Change `ingress-https-dev` (`sdd/changes/ingress-https-dev/`), decisiones D1–D10 de su `design.md`. Las decisiones D9 y D10 nacieron de hallazgos del panel de revisión durante la implementación, no del diseño inicial.
