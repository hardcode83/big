...
| El **host, por el gateway del bridge**: el puerto **22** | **No, y está MEDIDO** (2026-08-04, en la VM): desde la red de ingress se conecta a `<gateway>:22`. Publicarlo por el túnel saltaría el acotado por CIDR del security list. El mecanismo es una regla explícita del firewall de host — ver abajo |
| Por esa misma ruta, el **servicio de metadatos de la instancia** (`169.254.169.254`, link-local) y cualquier dirección **enrutable de la VCN** | **No, es la fila más grave, y está MEDIDA** (2026-08-04): desde la red de ingress se conecta a IMDS. Ya no es un riesgo esperado, es uno comprobado — ver abajo |
...
| `127.0.0.1:8000` / `127.0.0.1:3000` de `backend`/`frontend` | **Sí, y está MEDIDO** (2026-08-04): desde la red de ingress, `<gateway>:8000` y `:3000` devuelven `No route to host`. **Doblemente cubierto**, y el segundo motivo no estaba documentado: al binding en loopback se suma que el firewall de host **rechaza** ese tráfico (`icmp-host-prohibited`), porque su regla de `ACCEPT` general está acotada a `lo` |
...
**El mecanismo del residual, medido y no deducido** (2026-08-04, `iptables -L INPUT` en la VM). La imagen Ubuntu de Oracle trae un ruleset cuyo orden explica exactamente lo observado: `RELATED,ESTABLISHED` → `icmp` → un `ACCEPT` acotado a `lo` → **`ACCEPT tcp dpt:22` desde `0.0.0.0/0`** → **`REJECT … icmp-host-prohibited`**. Es decir: el 22 está abierto a *cualquier* origen, bridges de Docker incluidos, mientras que 8000/3000 caen en el REJECT final. IMDS es alcanzable por otra vía —no es tráfico de INPUT al host, sale enrutado— así que ninguna regla de INPUT lo cubre.

**Consecuencia práctica, y abarata la mitigación**: no hay que montar un firewall, ya existe uno. Basta insertar una regla **antes** de la del 22 que rechace ese puerto cuando el origen son las subredes de los bridges de Docker, dejando intacto el SSH de los operadores. Mitigar el residual —esa regla, más bloquear la link-local o desactivar los endpoints legacy de IMDS, y opcionalmente el endpoint de métricas del túnel— es un cambio de la **superficie de la VM**, no de la topología del compose, y queda registrado como candidato a change propio. Documentarlo con exactitud sí entra: repetir aquí el error de subestimar el radio habría sido especialmente caro, porque corregirlo es la razón de ser de `ingress-https-hardening`.
