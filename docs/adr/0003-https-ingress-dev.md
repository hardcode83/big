...
- **Los ajustes de HTTPS son de zona, no de hostname.** Cloudflare no los ofrece por hostname en el plan Free, así que forzar HTTPS y TLS mínimo 1.2 **aplica a todo `digitalsec.work`**, incluidos otros servicios del dominio. Aceptado a sabiendas: es la postura deseable para todos ellos. Si alguno necesitara HTTP en claro, este ajuste es lo primero a revisar.
