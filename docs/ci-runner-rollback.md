...
El fichero `runner-job-started.sh` en `$RUNNER_HOME/hooks/` se puede dejar instalado — sin la declaración en `.env`, GitHub nunca lo invoca; no hace falta borrarlo del disco para desactivarlo. Para reactivarlo, `RUNBOOK.md §6.2` documenta el paso a paso completo (copiar el hook, declarar la línea, reiniciar). **Ojo — reaplicar el bootstrap NO lo reactiva para un agente que este procedimiento haya desactivado**: `./config.sh --replace` falla con "Cannot configure the runner because it is already configured" contra un agente ya registrado (confirmado en la VM viva, change `ci-runner-workspace-pollution`, corregido también en `RUNBOOK.md §6.2` y `§7` de este mismo fichero), así que `install_job_started_hook`/`write_runner_env` nunca corren para él — el `.env` que este procedimiento dejó sin la línea sigue sin ella. Solo un agente que el bootstrap registra por primera vez en esa pasada recibe la línea automáticamente; para reactivar uno ya existente hace falta `RUNBOOK.md §6.2` a mano, como para desactivarlo.
