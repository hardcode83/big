...
export function GuestField({ id, label, value, error, onChange, children, as = "input", type = "text" }: Props) {
  ...
  return <div className="space-y-1"><label htmlFor={id} className="text-sm font-medium">{label}</label>{as === "textarea" ? <textarea {...common} rows={4} /> : as === "select" ? <select {...common}>{children}</select> : <input {...common} type={type} />}{error ? <p id={`${id}-error`} className="text-sm text-destructive">{error}</p> : null}</div>;
}
