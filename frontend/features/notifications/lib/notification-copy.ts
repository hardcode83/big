 ...
 * `backend/openapi.json`, so it is the twenty-four names the backend actually knows (measured
 * against the live enum after `guest-scheduled-comms` added `ACCESS_INSTRUCTIONS_SENT` —
 * re-measured, not incremented from the stale "twenty-two" this comment used to say). A `Record`
...
export const NOTIFICATION_COPY_KEYS: Record<NotificationType, string> = {
  ...
  ACCESS_INSTRUCTIONS_SENT: "notifications:types.ACCESS_INSTRUCTIONS_SENT",
};
