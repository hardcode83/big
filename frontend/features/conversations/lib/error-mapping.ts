...
/**
 * A discriminated union of the UI states the conversations views need to
 ...
 * typed DTO through to the consumer — a rename of `ConversationList` or
 * `ConversationDetailDto` propagates and tsc catches the consumer without an
 * `as` cast.
 */
export type ConversationsErrorState<TData> =
  | { kind: "loading" }
  | { kind: "forbidden" }
  | { kind: "not-found" }
  | { kind: "validation" }
 ...
 * conversations feature (R3.7, R6.4). Pure mapper — no React hooks
 * internally.
 *
 * Rules:
 ...
 */
export function mapConversationsError<TData, TError = Error>(
  queryResult: Pick<UseQueryResult<TData, TError>, "isPending" | "isError" | "error" | "data">,
): ConversationsErrorState<TData> {
