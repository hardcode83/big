...
function renderPagination(props: {
  ...
  const result = render(
    <I18nProvider locale="es">
      <ReviewsPagination {...props} onPageChange={onPageChange} />
    </I18nProvider>,
  );
...
describe("ReviewsPagination (R2.3, design D16)", () => {
  it("renders nothing at all when there are no pages (R2.3)", () => {
    const { container } = renderPagination({ page: 1, totalPages: 0, total: 0 });
  ...
  it("asks for the next and previous page without touching the network", () => {
    const { onPageChange } = renderPagination({ page: 2, totalPages: 3, total: 45 });
  ...
  it("takes its nav label from labelKey, so the two tabs differ", () => {
    renderPagination({
      page: 1,
      totalPages: 2,
      total: 30,
    ...
    expect(
      screen.getByRole("navigation", { name: "Paginación de borradores" }),
    ).toBeInTheDocument();
  });

  it("falls back to the generic label when none is passed", () => {
    renderPagination({ page: 1, totalPages: 2, total: 30 });
    expect(
      screen.getByRole("navigation", { name: "Paginación de reseñas" }),
    ).toBeInTheDocument();
  });
  ...
  it("has no accessibility violations", async () => {
    const { container } = renderPagination({ page: 2, totalPages: 3, total: 45 });
