...
/**
 * The two tabs of the technician's incident detail screen: **Incidencia** (the
 * operational content, active by default — R3.1) and **Mensajes** (design
 * D-mobile, D1). The mirror of `cleaner-task-tabs.tsx`, on the tech catalog.
 *
 ...
 * datos ya cargados" — and the content tab hosts the close form
 * (`tech-resolve-form.tsx`), the ETA field and the photo picker, whose local
 * state an unmount would erase. `hidden` also takes the inactive panel out of
 * the accessibility tree and out of the tab order, so the keyboard user never
 ...
 * technician asks for the thread (R2.1), and alternating tabs afterwards does
 * not take it away again (D1).
 */

export type TechIncidentTabKey = "content" | "messages";

const TAB_ORDER: readonly TechIncidentTabKey[] = ["content", "messages"];

const PANEL_IDS: Record<TechIncidentTabKey, string> = {
  content: "tech-incident-panel-content",
  messages: "tech-incident-panel-messages",
};

export interface TechIncidentTabsProps {
  /** The operational content of the incident — the default tab (R3.1). */
  content: ReactNode;
  /**
...
export function TechIncidentTabs({
  content,
  renderMessages,
}: TechIncidentTabsProps) {
  const { t } = useTranslation("tech");
  const [activeTab, setActiveTab] = useState<TechIncidentTabKey>("content");
  const [hasOpenedMessagesTab, setHasOpenedMessagesTab] = useState(false);
  const tabRefs = useRef<Record<string, HTMLButtonElement | null>>({});

  function selectTab(key: TechIncidentTabKey) {
    setActiveTab(key);
    // Sticky: once true it never returns to false, so the thread is not
    // re-disabled when the technician goes back to the content tab (D1).
  ...
  const panels: Record<TechIncidentTabKey, ReactNode> = {
    content,
    ...
    <div className="flex flex-col gap-4">
      ...
      {TAB_ORDER.map((key) => (
        <div
          ...
          role="tabpanel"
          id={PANEL_IDS[key]}
          aria-labelledby={`tech-incident-tab-${key}`}
