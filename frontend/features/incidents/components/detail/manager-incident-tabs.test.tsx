 ...
 * `ManagerIncidentActions` (whose sheet state R1.1 protects): if the tabs
 * unmounted the inactive panel, the count would be back at 0 after a round
 * trip through the messages tab.
 */
function Counter() {
...
const contentTab = () =>
  screen.getByRole("tab", { name: "Incidencia" });
const messagesTab = () =>
  screen.getByRole("tab", { name: "Mensajes" });

describe("ManagerIncidentTabs — structure and defaults (R1.1)", () => {
  ...
  it("opens on the operational content tab, not on messages (R1.1)", () => {
    renderTabs();
  ...
  it("uses the manager-incident-* DOM prefix to avoid tech/cleaner collisions (D2)", () => {
    renderTabs();
    expect(contentTab().getAttribute("id")).toBe("manager-incident-tab-content");
    expect(messagesTab().getAttribute("id")).toBe("manager-incident-tab-messages");
    expect(
      document.getElementById("manager-incident-panel-content"),
    ).not.toBeNull();
    expect(
      document.getElementById("manager-incident-panel-messages"),
    ).not.toBeNull();
  });
...
describe("ManagerIncidentTabs — both panels stay mounted (R1.1, design D3)", () => {
  ...
  it("preserves the content panel's local state across a round trip to messages (R1.1)", () => {
    renderTabs();
...
describe("ManagerIncidentTabs — lazy, sticky messages flag (D3)", () => {
...
describe("ManagerIncidentTabs — keyboard", () => {
