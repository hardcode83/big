  ...
  return (
    <form onSubmit={submit} aria-labelledby="reservation-create-title" className="flex flex-col gap-4 rounded-xl border border-border bg-surface p-4" noValidate>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        {(["checkInDate", "checkOutDate"] as const).map((key) => <div key={key}><label htmlFor={`reservation-create-${key}`} className="mb-1 block text-sm font-medium">{label(key)} ({t("edit.required")})</label>{input(key, "date", { required: true, "aria-required": true })}{fieldErrors[key] ? <p id={`reservation-create-${key}-error`} className="text-sm text-destructive">{fieldErrors[key]}</p> : null}</div>)}
        ...
        {(["adults", "children"] as const).map((key) => <div key={key}><label htmlFor={`reservation-create-${key}`} className="mb-1 block text-sm font-medium">{label(key)} ({t("edit.required")})</label>{input(key, "text", { inputMode: "numeric", required: true, "aria-required": true })}{fieldErrors[key] ? <p id={`reservation-create-${key}-error`} className="text-sm text-destructive">{fieldErrors[key]}</p> : null}</div>)}
